...
// // // // ═══════════════════════════════════════════════════════════════════
// // // // MUSIC ANALYZER ENGINE v7.1 — PAIR-AWARE UNIVERSAL ENGINE
// // // //
// // // // Root-cause fix from v7.0:
// // // //   v7.0 fingerprinted single bars → bar 3 ≠ bar 4 even though
// // // //   both are halves of one 2-bar Pattern A unit → wrong labels.
// // // //
// // // // v7.1 architecture:
// // // //   1. Multi-window fingerprinting (try pair=2 bars, then 1 bar).
// // // //      A "pair fingerprint" covers BOTH bars together: combined
// // // //      rhythm, full contour arc, anchor stability, density sum.
// // // //   2. Cluster on the winning window size.
// // // //   3. Assign BOTH bars in a pair the same label.
// // // //   4. Capsule + section detection same as v7.0.
// // // //
// // // // Result for Passacaglia:
// // // //   bars 3-4  → A   (one 2-bar pair)
// // // //   bars 5-6  → A   (same shape, different register → same family)
// // // //   bars 7-8  → A
// // // //   bars 9-10 → B   (chromatic sigh + long hold → new family)
// // // //   bars 11-12→ A   (ascending = high similarity to descending A)
// // // //   bars 43-48→ Aa  (parallel octaves = variant of A)
// // // //   Capsule   → (AAAB)x... correctly
// // // // ═══════════════════════════════════════════════════════════════════
...
// // //   const pcs  = new Set(allMidis.map(m => m % 12));
// // //   const root = rootMidi % 12;
// // //   const has  = iv => pcs.has((root + iv) % 12);
// // //   if (has(4) && has(7)) return NOTE_NAMES[root] + ' maj';
// // //   if (has(3) && has(7)) return NOTE_NAMES[root] + ' min';
// // //   if (has(4) && has(8)) return NOTE_NAMES[root] + ' aug';
// // //   if (has(3) && has(6)) return NOTE_NAMES[root] + ' dim';
...
// // // // ═══════════════════════════════════════════════════════════════════
// // // // FEATURE EXTRACTION — SINGLE BAR
// // // // ═══════════════════════════════════════════════════════════════════

// // // function extractBarFeatures(bar) {
// // //   const notes = [...bar.notes].sort((a, b) => a.start_subdivision - b.start_subdivision);

// // //   if (!notes.length) {
// // //     return {
// // //       density:0, rhythm:'', contour:'', intervals:[], durations:[],
// // //       register:0, anchor:null, motion:'empty', durationClass:'empty',
// // //       pitchSet:[], isEmpty:true,
// // //     };
// // //   }

// // //   const midis     = notes.map(n => pitchToMidi(n.pitch)).filter(Boolean);
// // //   const durations = notes.map(n => n.duration_subdivisions);
// // //   const rhythm    = durations.join('-');
// // //   const intervals = [];
// // //   for (let i = 1; i < midis.length; i++) intervals.push(midis[i] - midis[i - 1]);
// // //   const contour   = intervals.map(iv => iv > 1 ? '↑' : iv < -1 ? '↓' : '=').join('');
// // //   const register  = (Math.max(...midis) + Math.min(...midis)) / 2;
// // //   const anchor    = findAnchor(midis);
// // //   const allSame   = durations.every(d => d === durations[0]);
// // //   const durationClass = allSame
// // //     ? (durations[0] <= 2 ? 'short' : durations[0] <= 8 ? 'medium' : 'long')
// // //     : 'mixed';
// // //   const pitchSet  = [...new Set(midis.map(m => m % 12))].sort((a, b) => a - b);

// // //   return {
// // //     density: notes.length, rhythm, contour, intervals, durations,
// // //     register, anchor, motion: classifyMotion(intervals),
// // //     durationClass, pitchSet, isEmpty: false,
// // //   };
// // // }

// // // function findAnchor(midis) {
// // //   const map = new Map();
// // //   for (const m of midis) map.set(m, (map.get(m) || 0) + 1);
// // //   let best = null, count = 0;
// // //   for (const [m, c] of map) { if (c > count) { count = c; best = m; } }
// // //   return best;
// // // }

// // // function classifyMotion(intervals) {
// // //   const up   = intervals.filter(x => x > 0).length;
// // //   const dn   = intervals.filter(x => x < 0).length;
// // //   if (up > dn * 2)  return 'ascending';
// // //   if (dn > up * 2)  return 'descending';
// // //   return 'oscillating';
// // // }


// // // // ═══════════════════════════════════════════════════════════════════
// // // // FEATURE EXTRACTION — 2-BAR PAIR (the key fix)
// // // //
// // // // Merges two consecutive bars into one fingerprint that captures
// // // // the full melodic arc (both halves of Pattern A together).
// // // // ═══════════════════════════════════════════════════════════════════

// // // function extractPairFeatures(bar1, bar2, spb) {
// // //   const SPB = spb || 16;
// // //   // combine notes, offset bar2 by SPB
// // //   const allNotes = [
// // //     ...[...bar1.notes].map(n => ({ ...n })),
// // //     ...[...bar2.notes].map(n => ({ ...n, start_subdivision: n.start_subdivision + SPB })),
// // //   ].sort((a, b) => a.start_subdivision - b.start_subdivision);
...
// // //       density:0, rhythm:'', contour:'', intervals:[], durations:[],
// // //       register:0, anchor:null, motion:'empty', durationClass:'empty',
// // //       pitchSet:[], isEmpty:true, windowSize:2,
// // //     };
// // //   }

// // //   const midis     = allNotes.map(n => pitchToMidi(n.pitch)).filter(Boolean);
// // //   const durations = allNotes.map(n => n.duration_subdivisions);
// // //   const rhythm    = durations.join('-');
// // //   const intervals = [];
// // //   for (let i = 1; i < midis.length; i++) intervals.push(midis[i] - midis[i - 1]);
// // //   const contour   = intervals.map(iv => iv > 1 ? '↑' : iv < -1 ? '↓' : '=').join('');
...
// // //   // anchor = most repeated MIDI in the combined window
// // //   const anchor    = findAnchor(midis);

// // //   // Determine if one voice repeats (= alternating-weave pattern)
// // //   const anchorCount = midis.filter(m => m === anchor).length;
// // //   const anchorRatio = anchorCount / midis.length;

// // //   // Duration class from combined rhythm
// // //   const allSame   = durations.every(d => d === durations[0]);
// // //   const durationClass = allSame
// // //     ? (durations[0] <= 2 ? 'short' : durations[0] <= 8 ? 'medium' : 'long')
// // //     : 'mixed';
...
// // //     register,
// // //     anchor,
// // //     anchorRatio,  // high = alternating weave (Pattern A style)
// // //     motion:       classifyMotion(intervals),
// // //     durationClass,
// // //     pitchSet,
// // //     isEmpty:      false,
// // //     windowSize:   2,
...
// // // // ═══════════════════════════════════════════════════════════════════
// // // // SIMILARITY — works for both single-bar and pair fingerprints
// // // // ═══════════════════════════════════════════════════════════════════
...
// // //   // Rhythm string — strongest structural signal
// // //   if (a.rhythm === b.rhythm) score += 0.30;
// // //   else {
// // //     // partial credit for same duration class (e.g. both all-d=2)
// // //     if (a.durationClass === b.durationClass) score += 0.10;
// // //   }

// // //   // Contour — direction of melody arc
// // //   // Full match: full credit. Mirror (ascending vs descending): partial credit.
// // //   if (a.contour === b.contour) {
// // //     score += 0.22;
// // //   } else {
// // //     // Mirror check: reverse one contour and flip ↑↓
// // //     const mirrorA = a.contour.split('').reverse().map(c => c === '↑' ? '↓' : c === '↓' ? '↑' : '=').join('');
// // //     if (mirrorA === b.contour) score += 0.14; // inverted = strong variant, not new family
// // //     else {
// // //       // partial: same length contour, mostly matching
// // //       if (a.contour.length === b.contour.length) {
// // //         let matches = 0;
// // //         for (let i = 0; i < a.contour.length; i++)
// // //           if (a.contour[i] === b.contour[i]) matches++;
// // //         score += 0.10 * (matches / a.contour.length);
// // //       }
// // //     }
// // //   }

// // //   // Duration class
// // //   if (a.durationClass === b.durationClass) score += 0.12;

// // //   // Density similarity
// // //   const maxDen = Math.max(a.density, b.density);
// // //   if (maxDen > 0) score += 0.10 * (1 - Math.abs(a.density - b.density) / maxDen);

// // //   // Anchor ratio (weave vs sparse) — important for A vs B distinction
// // //   if (a.anchorRatio !== undefined && b.anchorRatio !== undefined) {
// // //     score += 0.10 * (1 - Math.abs(a.anchorRatio - b.anchorRatio));
// // //   } else {
// // //     score += 0.05;
// // //   }

// // //   // Register closeness (±1 octave = full credit)
// // //   const regDiff = Math.abs(a.register - b.register);
// // //   score += 0.08 * Math.max(0, 1 - regDiff / 12);

// // //   // Pitch class set Jaccard
// // //   const setA = new Set(a.pitchSet), setB = new Set(b.pitchSet);
// // //   const inter = [...setA].filter(x => setB.has(x)).length;
// // //   const union = new Set([...setA, ...setB]).size;
// // //   if (union > 0) score += 0.08 * (inter / union);
...
// // // // PATTERN CLUSTER
// // // // Threshold tuning:
// // // //   ≥ 0.88 → same family (exact or inverted = still "A")
// // // //   ≥ 0.72 → variant-a  (e.g. parallel octaves version of A)
// // // //   ≥ 0.55 → variant-b  (loose variant)
// // // //   <  0.55 → new family
// // // // ═══════════════════════════════════════════════════════════════════
...
// // //   addFeatures(fp, barNumbers) {
// // //     // silence
// // //     if (fp.isEmpty) {
// // //       let sil = this.families.find(f => f.id === 'S');
// // //       if (!sil) {
// // //         sil = { id:'S', prototype:fp, members:[] };
// // //         this.families.push(sil);
// // //       }
// // //       sil.members.push({ fp, barNumbers });
// // //       return 'S';
// // //     }

// // //     let best = null, bestScore = 0;
// // //     for (const fam of this.families) {
// // //       if (fam.id === 'S') continue;
// // //       const s = featureSimilarity(fam.prototype, fp);
// // //       if (s > bestScore) { best = fam; bestScore = s; }
// // //     }

// // //     if (bestScore >= T_EXACT) {
// // //       best.members.push({ fp, barNumbers });
// // //       return best.id;
// // //     }
// // //     if (bestScore >= T_NEAR_A) {
// // //       best.members.push({ fp, barNumbers });
// // //       return best.id + 'a';
// // //     }
// // //     if (bestScore >= T_NEAR_B) {
// // //       best.members.push({ fp, barNumbers });
// // //       return best.id + 'b';
// // //     }

// // //     const baseCount = this.families.filter(f => f.id !== 'S').length;
// // //     const newId = String.fromCharCode(65 + baseCount);
// // //     this.families.push({ id:newId, prototype:fp, members:[{ fp, barNumbers }] });
// // //     return newId;
// // //   }
// // // }
...
// // // // WINDOW SELECTOR
// // // //
// // // // Decides whether to cluster this track at pair (2-bar) or
// // // // single-bar granularity.
// // // //
// // // // Heuristic: if the majority of content bars have exactly the same
// // // // rhythm string as their neighbour (typical of 2-bar alternating-
// // // // weave patterns), prefer pair-level clustering.
// // // // ═══════════════════════════════════════════════════════════════════

// // // function choosePairWindow(bars, spb) {
// // //   const content = bars.filter(b => b.notes.length > 0);
// // //   if (content.length < 4) return false;

// // //   let pairSame = 0;
// // //   for (let i = 0; i < content.length - 1; i += 2) {
// // //     const a = extractBarFeatures(content[i]);
// // //     const b = extractBarFeatures(content[i + 1]);
// // //     // Same density + same duration class = likely 2-bar unit
// // //     if (a.durationClass === b.durationClass && a.density === b.density) pairSame++;
// // //   }
// // //   const pairRatio = pairSame / Math.floor(content.length / 2);
// // //   return pairRatio >= 0.5; // majority of pairs look like 2-bar units
// // // }
...
// // // function detectCapsules(labels) {
// // //   const result = [];
// // //   let i = 0;
// // //   while (i < labels.length) {
// // //     let found = false;
// // //     for (let size = Math.min(8, Math.floor((labels.length - i) / 2)); size >= 2; size--) {
// // //       const chunk = labels.slice(i, i + size).join('');
// // //       if (chunk.length < size) continue;
// // //       let repeats = 1;
// // //       while (
// // //         i + repeats * size + size <= labels.length &&
// // //         labels.slice(i + repeats * size, i + (repeats + 1) * size).join('') === chunk
// // //       ) { repeats++; }
// // //       if (repeats > 1) {
// // //         result.push({ pattern:chunk, repeats, startIdx:i, endIdx:i + size*repeats - 1 });
// // //         i += size * repeats;
// // //         found = true;
// // //         break;
// // //       }
// // //     }
// // //     if (!found) {
// // //       result.push({ pattern:labels[i], repeats:1, startIdx:i, endIdx:i });
// // //       i++;
// // //     }
// // //   }
// // //   return result;
...
// // // // ═══════════════════════════════════════════════════════════════════
// // // // SECTION DETECTION (capsule-level Jaccard boundary)
// // // // ═══════════════════════════════════════════════════════════════════

// // // function capsuleJaccard(a, b) {
// // //   const setA = new Set([...a.pattern]);
// // //   const setB = new Set([...b.pattern]);
// // //   const inter = [...setA].filter(x => setB.has(x)).length;
// // //   const union  = new Set([...setA, ...setB]).size;
// // //   return union > 0 ? inter / union : 0;
// // // }

// // // function detectSections(capsules, unitBars) {
// // //   if (!capsules.length) return [];
// // //   const sections = [];
// // //   let current = [capsules[0]];

// // //   for (let c = 1; c < capsules.length; c++) {
// // //     const sim = capsuleJaccard(current[current.length - 1], capsules[c]);
// // //     if (sim < 0.45) { sections.push(current); current = [capsules[c]]; }
// // //     else current.push(capsules[c]);
// // //   }
// // //   sections.push(current);

// // //   const ROMAN = ['I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII','XIII','XIV','XV','XVI'];
// // //   return sections.map((caps, i) => {
// // //     const firstUnit = caps[0].startIdx;
// // //     const lastUnit  = caps[caps.length - 1].endIdx;
// // //     const startBar  = unitBars[firstUnit]?.[0] ?? firstUnit + 1;
// // //     const endBar    = unitBars[Math.min(lastUnit, unitBars.length - 1)]?.slice(-1)[0] ?? lastUnit + 1;

// // //     const freq = new Map();
// // //     caps.forEach(cap => {
// // //       const base = cap.pattern.replace(/[ab]/g,'');
// // //       freq.set(base, (freq.get(base)||0) + cap.repeats);
// // //     });
// // //     let dominant = '?', domCt = 0;
// // //     for (const [p,ct] of freq) { if (ct > domCt) { dominant=p; domCt=ct; } }

// // //     const allLabels  = caps.flatMap(c => Array.from(c.pattern));
// // //     const uniquePats = [...new Set(allLabels)];
// // //     const hasVar     = allLabels.some(l => l.endsWith('a') || l.endsWith('b'));
// // //     const allSil     = allLabels.every(l => l === 'S');
// // //     const capsStr    = caps.map(c => c.repeats>1 ? `(${c.pattern})x${c.repeats}` : c.pattern).join(' ');

// // //     let type;
// // //     if (allSil) type = 'Intro';
// // //     else if (i === sections.length - 1 && caps.length === 1 && caps[0].repeats === 1) type = 'Coda';
// // //     else if (hasVar) type = 'Development';
// // //     else if (uniquePats.length === 1) type = 'Ostinato';
// // //     else type = 'Section';

// // //     return {
// // //       index:i, romanLabel:ROMAN[i]||`S${i+1}`, type,
// // //       startBar, endBar, capsules:caps,
// // //       dominantPattern:dominant, uniquePatterns:uniquePats, capsuleStr:caps.map(c=>c.repeats>1?`(${c.pattern})x${c.repeats}`:c.pattern).join(" "),
// // //     };
// // //   });
// // // }
...
// // // // ═══════════════════════════════════════════════════════════════════
// // // // MELODY DIRECTION — regression slope (for per-bar detail labels)
// // // // ═══════════════════════════════════════════════════════════════════

// // // function getMelodyDirection(bar, anchorMidi) {
// // //   const notes = [...bar.notes]
// // //     .sort((a,b)=>a.start_subdivision-b.start_subdivision)
// // //     .filter(n => pitchToMidi(n.pitch) !== anchorMidi);
// // //   if (notes.length < 2) return 'static';
// // //   const pts = notes.map(n=>({x:n.start_subdivision, y:pitchToMidi(n.pitch)})).filter(p=>p.y!==null);
// // //   if (pts.length < 2) return 'static';
...
// // // // ═══════════════════════════════════════════════════════════════════
// // // // FAMILY BUILDER — produces frontend-compatible family records
// // // // ═══════════════════════════════════════════════════════════════════

// // // const PATTERN_COLORS = [
// // //   {bg:'#3b82f6',text:'#fff'}, // A — blue
// // //   {bg:'#8b5cf6',text:'#fff'}, // B — purple
// // //   {bg:'#10b981',text:'#001'}, // C — green
// // //   {bg:'#ef4444',text:'#fff'}, // D — red
// // //   {bg:'#f59e0b',text:'#001'}, // E — amber
// // //   {bg:'#ec4899',text:'#fff'}, // F — pink
// // //   {bg:'#06b6d4',text:'#001'}, // G — cyan
// // //   {bg:'#84cc16',text:'#001'}, // H — lime
// // // ];

// // // function buildFamilies(labels, unitBars) {
// // //   // unitBars[i] = array of bar_numbers covered by label[i]
// // //   const byLabel = new Map();
// // //   labels.forEach((lbl, idx) => {
// // //     if (!byLabel.has(lbl)) byLabel.set(lbl, []);
// // //     byLabel.get(lbl).push({ idx, barNumbers: unitBars[idx] });
// // //   });

// // //   const sortedLabels = [...byLabel.keys()].sort((a, b) => {
// // //     if (a==='S') return 1; if (b==='S') return -1;
// // //     const ba=a.replace(/[ab]$/,''), bb=b.replace(/[ab]$/,'');
// // //     if (ba!==bb) return ba.localeCompare(bb);
// // //     return a.localeCompare(b);
// // //   });

// // //   return sortedLabels.map(lbl => {
// // //     const members = byLabel.get(lbl);
// // //     const baseId  = lbl.replace(/[ab]$/,'');
// // //     const baseIdx = baseId==='S' ? 8 : baseId.charCodeAt(0)-65;
// // //     const color   = PATTERN_COLORS[baseIdx % PATTERN_COLORS.length] ?? {bg:'#6b7280',text:'#fff'};
// // //     const allBarNumbers = members.flatMap(m => m.barNumbers);

// // //     return {
// // //       id:              `FAM_${lbl}`,
// // //       label:           lbl==='S' ? 'Silence/Intro' : `Pattern ${lbl}`,
// // //       role:            lbl==='S' ? 'silence' : lbl.endsWith('b') ? 'variant-weak'
// // //         : lbl.endsWith('a') ? 'variant-strong' : 'primary',
// // //       patternLabel:    lbl,
// // //       matchLevel:      'structural',
// // //       occurrenceCount: members.length,
// // //       occurrences:     members.map((m,i) => ({
// // //         unitIndex: m.idx, barNumbers: m.barNumbers,
// // //         startBar: m.barNumbers[0], endBar: m.barNumbers[m.barNumbers.length-1],
// // //         barRange: m.barNumbers.length===1 ? `${m.barNumbers[0]}` : `${m.barNumbers[0]}-${m.barNumbers[m.barNumbers.length-1]}`,
// // //         variationType: i===0 ? 'prototype' : 'variation',
// // //       })),
// // //       barNumbers:      allBarNumbers,
// // //       color:           lbl==='S' ? {bg:'#1f2937',text:'#4b5563'} : color,
// // //       windowSize:      unitBars[0]?.length ?? 1,
// // //     };
// // //   });
// // // }
...
// // //   if (!bars || !bars.length) return emptyTrackResult();
// // //   const SPB = spb || 16;

// // //   const usePairs = choosePairWindow(bars, SPB);
// // //   const cluster  = new PatternCluster();
// // //   const labels   = [];
// // //   const unitBars = []; // unitBars[i] = [bar_number, ...] for label i

// // //   if (usePairs) {
// // //     // Process bars in pairs; single trailing bar gets solo treatment
// // //     let i = 0;
// // //     while (i < bars.length) {
// // //       const bar1 = bars[i];
// // //       const bar2 = bars[i + 1];

// // //       // If both bars empty → one silence unit
// // //       if (!bar1.notes.length && (!bar2 || !bar2.notes.length)) {
// // //         const fp = extractPairFeatures(bar1, bar2 || bar1, SPB);
// // //         const lbl = cluster.addFeatures(fp, [bar1.bar_number, ...(bar2 ? [bar2.bar_number] : [])]);
// // //         if (bar2) {
// // //           labels.push(lbl); unitBars.push([bar1.bar_number]);
// // //           labels.push(lbl); unitBars.push([bar2.bar_number]);
// // //         } else {
// // //           labels.push(lbl); unitBars.push([bar1.bar_number]);
// // //         }
// // //         i += bar2 ? 2 : 1;
...
// // //       // If one bar empty and one not, try as pair anyway (intro case)
// // //       if (bar2 && (bar1.notes.length === 0 || bar2.notes.length === 0)) {
// // //         // Solo-label each separately
// // //         const fp1 = extractBarFeatures(bar1);
// // //         const lbl1 = cluster.addFeatures(fp1, [bar1.bar_number]);
// // //         labels.push(lbl1); unitBars.push([bar1.bar_number]);
// // //         i++;
// // //         continue;
// // //       }
...
// // //       if (!bar2) {
// // //         // trailing single bar
// // //         const fp = extractBarFeatures(bar1);
// // //         const lbl = cluster.addFeatures(fp, [bar1.bar_number]);
// // //         labels.push(lbl); unitBars.push([bar1.bar_number]);
// // //         i++;
// // //         continue;
// // //       }
...
// // //       // Normal 2-bar pair
// // //       const fp  = extractPairFeatures(bar1, bar2, SPB);
// // //       const lbl = cluster.addFeatures(fp, [bar1.bar_number, bar2.bar_number]);
// // //       labels.push(lbl); unitBars.push([bar1.bar_number]);
// // //       labels.push(lbl); unitBars.push([bar2.bar_number]);
// // //       i += 2;
// // //     }
...
// // //     // Single-bar mode
// // //     for (const bar of bars) {
// // //       const fp  = extractBarFeatures(bar);
// // //       const lbl = cluster.addFeatures(fp, [bar.bar_number]);
// // //       labels.push(lbl); unitBars.push([bar.bar_number]);
// // //     }
// // //   }

// // //   const capsules = detectCapsules(labels);
// // //   const sections = detectSections(capsules, unitBars);
// // //   const families = buildFamilies(labels, unitBars);

// // //   // bar-level labeled list
// // //   const famByLabel = new Map(families.map(f => [f.patternLabel, f]));
// // //   const labeled = bars.map((bar, idx) => {
// // //     const lbl  = labels[idx];
// // //     const fam  = famByLabel.get(lbl);
// // //     const feat = extractBarFeatures(bar);
// // //     const sec  = sections.find(s => bar.bar_number >= s.startBar && bar.bar_number <= s.endBar);
// // //     return {
// // //       barNumber:     bar.bar_number,
// // //       notes:         bar.notes,
// // //       patternLabel:  lbl,
// // //       patternId:     fam?.id ?? `FAM_${lbl}`,
// // //       role:          fam?.role ?? 'primary',
// // //       sectionLabel:  sec?.romanLabel ?? null,
// // //       sectionType:   sec?.type ?? null,
// // //       isEmpty:       feat.isEmpty,
// // //       density:       feat.density,
// // //       motion:        feat.motion,
// // //       durationClass: feat.durationClass,
// // //       register:      feat.register,
// // //       anchor:        feat.anchor ? midiToName(feat.anchor) : null,
// // //       melodyDirection: feat.anchor ? getMelodyDirection(bar, feat.anchor) : null,
// // //       color:         fam?.color ?? {bg:'#6b7280',text:'#fff'},
// // //       noteCount:     bar.notes.length,
// // //     };
// // //   });
...
// // //   // graph
// // //   const nodeMap = new Map();
// // //   for (const s of sections) nodeMap.set(s.romanLabel, { id:s.romanLabel, label:`${s.romanLabel} — ${s.type}`, count:s.endBar-s.startBar+1, color:'#6366f1' });
// // //   const edgeMap = new Map();
// // //   for (let i = 0; i < sections.length-1; i++) {
// // //     const key = `${sections[i].romanLabel}->${sections[i+1].romanLabel}`;
// // //     edgeMap.set(key, { from:sections[i].romanLabel, to:sections[i+1].romanLabel, weight:(edgeMap.get(key)?.weight??0)+1 });
// // //   }
// // //   const graph = { nodes:[...nodeMap.values()], edges:[...edgeMap.values()].sort((a,b)=>b.weight-a.weight) };

// // //   const boundaries = sections.slice(0,-1).map((s,i)=>({
// // //     afterBarNumber: s.endBar, type: sections[i+1].type,
// // //     noveltyScore:   capsuleJaccard(s.capsules[s.capsules.length-1], sections[i+1].capsules[0]),
// // //   }));

// // //   return { labels, families, labeled, sections, capsules, graph, boundaries, unitBars, usePairs, clusterFamilies:cluster.families };
// // // }

// // // function emptyTrackResult() {
// // //   return { labels:[], families:[], labeled:[], sections:[], capsules:[], graph:{nodes:[],edges:[]}, boundaries:[], unitBars:[], usePairs:false, clusterFamilies:[] };
// // // }


// // // // ═══════════════════════════════════════════════════════════════════
// // // // LEFT HAND — chord labeling layer
// // // // ═══════════════════════════════════════════════════════════════════
...
// // // function analyzeLeftHand(bars, spb) {
// // //   const base = analyzeTrack(bars, spb);
// // //   const augLabeled = base.labeled.map(lb => {
// // //     const bar = bars.find(b=>b.bar_number===lb.barNumber);
// // //     return { ...lb, chord: bar ? lhChordLabel(bar) : null };
// // //   });

// // //   // Chord cycle detection — only on arpeggio bars (8 notes d<=4), excluding coda held octaves
// // //   const arpeggioLabeled = augLabeled.filter(lb => {
// // //     const bar = bars.find(b=>b.bar_number===lb.barNumber);
// // //     if (!bar) return false;
// // //     return bar.notes.length >= 6 && bar.notes.every(n=>n.duration_subdivisions<=4);
// // //   });
// // //   const arpChords = arpeggioLabeled.filter(lb=>lb.chord).map(lb=>lb.chord);
// // //   let ostinato    = null;
// // //   // Try finding cycle from each starting offset (handles intro bars before main cycle)
// // //   for (let startOffset=0; startOffset<4 && !ostinato; startOffset++) {
// // //     const chords = arpChords.slice(startOffset);
// // //     for (let cycLen=4; cycLen<=12; cycLen++) {
// // //       const cycle=chords.slice(0,cycLen).join(',');
// // //       let reps=1;
// // //       while (reps*cycLen<chords.length && chords.slice(reps*cycLen,(reps+1)*cycLen).join(',')===cycle) reps++;
// // //       if (reps>=2) { ostinato={cycle:chords.slice(0,cycLen),cycleLength:cycLen,repeatCount:reps,introOffset:startOffset}; break; }
// // //     }
// // //   }

// // //   // Chord families for frontend
// // //   const chordMap = new Map();
// // //   for (const lb of augLabeled) {
// // //     if (!lb.chord) continue;
// // //     if (!chordMap.has(lb.chord)) chordMap.set(lb.chord,[]);
// // //     chordMap.get(lb.chord).push(lb);
// // //   }
// // //   const lhChordFamilies = [...chordMap.entries()].map(([chord,members])=>({
// // //     id:`LH_${chord.replace(/\s/g,'_')}`, label:chord, role:'chord', chord,
// // //     matchLevel:'structural', occurrenceCount:members.length,
// // //     occurrences:members.map(m=>({barNumber:m.barNumber,startBar:m.barNumber,endBar:m.barNumber,barRange:`${m.barNumber}`,variationType:'exact'})),
// // //     barNumbers:members.map(m=>m.barNumber),
// // //     color:{bg:'#8b5cf6',text:'#fff'}, windowSize:1,
// // //   }));

// // //   return { hand:'left', ...base, labeled:augLabeled, families:[...base.families,...lhChordFamilies], patterns:[...base.families,...lhChordFamilies], ostinato };
// // // }
...
// // // function analyzeRightHand(bars, spb) {
// // //   const result = analyzeTrack(bars, spb);
// // //   const period = inferPeriod(result.capsules);
// // //   return {
// // //     hand:'right', ...result, patterns:result.families,
// // //     introBars: bars.filter((_,i)=>result.labels[i]==='S'),
// // //     period, periodConfidence:inferPeriodConfidence(result.capsules),
// // //     _sections: result.sections,
// // //   };
// // // }

// // // function inferPeriod(capsules) {
// // //   if (!capsules.length) return null;
// // //   const sc = new Map();
// // //   for (const c of capsules) { const sz=c.pattern.length; sc.set(sz,(sc.get(sz)||0)+c.repeats); }
// // //   let best=null, bestCt=0;
// // //   for (const [sz,ct] of sc) { if (ct>bestCt) { bestCt=ct; best=sz; } }
// // //   return best;
// // // }

// // // function inferPeriodConfidence(capsules) {
// // //   const p = inferPeriod(capsules);
// // //   if (p===null) return null;
// // //   const total   = capsules.reduce((a,c)=>a+c.repeats,0);
// // //   const match   = capsules.filter(c=>c.pattern.length===p).reduce((a,c)=>a+c.repeats,0);
...
// // // function generateYaml(rhResult, lhResult, metadata) {
// // //   const sections   = rhResult._sections || rhResult.sections || [];
// // //   const capStr     = (rhResult.capsules||[]).map(c=>c.repeats>1?`(${c.pattern})x${c.repeats}`:c.pattern).join(' ');
// // //   const period     = rhResult.period ?? 'auto';
// // //   const confidence = typeof rhResult.periodConfidence==='number' ? rhResult.periodConfidence.toFixed(2) : 'n/a';

// // //   const secLines = sections.map(s =>
// // //     `    ${s.romanLabel} (${s.type}): {bars: [${s.startBar}, ${s.endBar}], capsules: "${s.capsuleStr}", patterns: [${s.uniquePatterns.join(', ')}]}`
// // //   ).join('\n');

// // //   const famLines = (rhResult.families||[])
// // //     .filter(f=>f.role!=='silence')
// // //     .map(f=>`    ${f.label}:\n      count: ${f.occurrenceCount}\n      match: "${f.matchLevel}"\n      bars: [${f.barNumbers.slice(0,20).join(', ')}${f.barNumbers.length>20?'...':''}]`)
// // //     .join('\n\n');

// // //   const lhCycle = lhResult?.ostinato?.cycle?.join(' → ') ?? 'n/a';
// // //   const lhFams  = (lhResult?.families||[]).filter(f=>f.role==='chord')
// // //     .map(f=>`    ${f.label}: {count: ${f.occurrenceCount}, bars: [${f.barNumbers.slice(0,10).join(', ')}${f.barNumbers.length>10?'...':''}]}`)
// // //     .join('\n');

// // //   return `# PASSACAGLIA ANALYSIS BLUEPRINT
// // // # ${metadata.bars.length} bars · ${metadata.time_signature} · ${metadata.tempo} BPM · Key: ${metadata.key}
// // // # Sections: ${sections.length} · Dominant period: ${period} bars · Window: ${rhResult.usePairs?'2-bar pairs':'1-bar'}
...
// // //   clustering_window: ${rhResult.usePairs?'pair (2 bars)':'single (1 bar)'}

// // // right_hand:
// // //   capsule_sequence: "${capStr}"
...
// // //   if (input.full) {
// // //     const norm=normalizeJson(input.full); metadata=norm; spb=norm.subdivisions_per_bar;
// // //     splitMidi=input.splitMidi ?? detectSplitMidi(norm.bars);
// // //     const split=splitHandsFromFull(norm.bars,splitMidi);
// // //     rhBars=split.rhBars; lhBars=split.lhBars; splitMidi=split.splitMidi;
// // //   } else {
// // //     if (input.rh) { const norm=normalizeJson(input.rh); metadata=metadata??norm; spb=norm.subdivisions_per_bar; rhBars=norm.bars; }
// // //     if (input.lh) { const norm=normalizeJson(input.lh); metadata=metadata??norm; lhBars=norm.bars; }
// // //     if (!metadata && (rhBars||lhBars)) metadata={tempo:120,time_signature:'4/4',key:'C',subdivisions_per_bar:16,bars:rhBars||lhBars};
// // //   }

// // //   if (!metadata) throw new Error('No input provided. Pass { rh }, { lh }, { rh, lh }, or { full }.');

// // //   const rhResult = rhBars ? analyzeRightHand(rhBars, spb) : emptyTrackResult();
// // //   const lhResult = lhBars ? analyzeLeftHand(lhBars, spb)  : null;

// // //   const allBns   = new Set([...(rhResult.labeled||[]).map(b=>b.barNumber),...(lhResult?.labeled||[]).map(b=>b.barNumber)]);
// // //   const rhMap    = new Map((rhResult.labeled||[]).map(b=>[b.barNumber,b]));
// // //   const lhMap    = new Map((lhResult?.labeled||[]).map(b=>[b.barNumber,b]));
// // //   const alignment= [...allBns].sort((a,b)=>a-b).map(bn=>({barNumber:bn,rh:rhMap.get(bn)??null,lh:lhMap.get(bn)??null}));

// // //   const yaml = generateYaml(rhResult, lhResult??{}, metadata);
// // //   const splitDisplay = splitMidi ?? (rhBars&&lhBars ? detectSplitMidi([...rhBars,...lhBars]) : null);

// // //   return {
// // //     metadata, splitMidi:splitDisplay,
// // //     rightHand:{ families:rhResult.families, patterns:rhResult.families, labeled:rhResult.labeled, sections:rhResult.sections, capsules:rhResult.capsules, graph:rhResult.graph, boundaries:rhResult.boundaries },
// // //     leftHand: lhResult ? { families:lhResult.families, patterns:lhResult.families, labeled:lhResult.labeled, sections:lhResult.sections, capsules:lhResult.capsules, graph:lhResult.graph, ostinato:lhResult.ostinato }
// // //       : { families:[],patterns:[],labeled:[],sections:[],capsules:[],graph:{nodes:[],edges:[]},ostinato:null },
// // //     alignment, yamlBlueprint:yaml,
// // //     summary:{
// // //       totalBars:metadata.bars.length,
// // //       detectedPeriod:rhResult.period,
// // //       periodConfidence:rhResult.periodConfidence,
// // //       clusteringWindow:rhResult.usePairs?'2-bar pairs':'1-bar',
// // //       rhSections:(rhResult.sections||[]).length,
// // //       lhChordCycle:lhResult?.ostinato?.cycle??[],
// // //       rhFamilies:(rhResult.families||[]).length,
// // //       lhFamilies:lhResult?.families?.length??0,
// // //       capsuleSequence:(rhResult.capsules||[]).map(c=>c.repeats>1?`(${c.pattern})x${c.repeats}`:c.pattern).join(' '),
// // //       splitMidi:splitDisplay,
// // //     },
// // //   };
// // // }
...
// // // const MusicAnalyzerEngine = {
// // //   analyze, analyzeRightHand, analyzeLeftHand, analyzeTrack,
// // //   normalizeJson, normalizeNote, normalizeBar,
// // //   splitHandsFromFull, detectSplitMidi,
// // //   extractBarFeatures, extractPairFeatures, featureSimilarity,
// // //   PatternCluster, choosePairWindow,
// // //   detectCapsules, detectSections, buildFamilies,
// // //   inferPeriod, inferPeriodConfidence,
// // //   pitchToMidi, midiToName, pitchClass, chordName,
// // //   getMelodyDirection, generateYaml,
// // // };
...
// // // ═══════════════════════════════════════════════════════════════════
// // // MUSIC ANALYZER ENGINE v8.0 — MOTIF-FIRST DSA ENGINE
// // //
// // // Core algorithm (no clustering, no thresholds, no pair-windows):
// // //
// // //   1. barFingerprint(bar)
// // //      → (noteCount | durString | simultaneous?)
// // //      The single key that identifies a musical texture.
// // //      Same rhythm at any pitch = same fingerprint.
// // //
// // //   2. findMotifs(bars)
// // //      → count fingerprint frequency across all bars
// // //      → sort by frequency (most bars = label A, next = B, ...)
// // //      → bars with unique fingerprint (≤1 occurrence) = label X
// // //      → empty bars = label S
// // //
// // //   3. detectSections(labeled)
// // //      → RLE compress label sequence
// // //      → short runs (≤2 bars) get absorbed into adjacent major runs
// // //      → section boundary = long run of genuinely different motif
// // //      → type: Silence / Exposition / Development / Reprise / Coda
// // //
// // //   4. generateYaml
// // //      → clean, minimal YAML: motifs + sections only
// // //
// // // This works for ANY music style — not passacaglia-specific.
// // // Deterministic, no floating-point comparison.
// // // ═══════════════════════════════════════════════════════════════════
...
// // // ═══════════════════════════════════════════════════════════════════
// // // BAR FINGERPRINT — the core DSA primitive
// // //
// // // fingerprint key = "noteCount|durString|sim"
// // //   noteCount  = how many notes in the bar
// // //   durString  = durations joined with '-', in start_subdivision order
// // //   sim        = '1' if any two notes share a start_subdivision, else '0'
// // //
// // // Same texture at any pitch → same fingerprint.
// // // This is purely rhythmic identity, pitch-agnostic.
// // //
// // // rhythmClass = coarse texture tag derived from the fingerprint:
// // //   empty     → no notes
// // //   rapid     → all d=1 (16th notes)
// // //   weave     → all d=2, no simultaneous (8th-note alternating weave)
// // //   interval  → all d=2, simultaneous pairs (interval pairs)
// // //   sustain   → very few notes (≤3) with long durations (≥ spb/2)
// // //   lyrical   → few notes (≤6) with at least one long note
// // //   pulse     → all same duration (other cases)
// // //   mixed     → varied durations
// // // ═══════════════════════════════════════════════════════════════════
...
// //   if (!notes.length) {
// //     return {
// //       noteCount: 0, durString: '', isUniform: true, uniformDur: null,
// //       maxDur: 0, minDur: 0, hasSimultaneous: false, isEmpty: true,
// //       rhythmClass: 'empty', fingerprint: 'EMPTY',
// //     };
// //   }

// //   // Detect simultaneous notes (same start_subdivision)
// //   const subMap = new Map();
// //   for (const n of notes)
// //     subMap.set(n.start_subdivision, (subMap.get(n.start_subdivision) || 0) + 1);
// //   const hasSimultaneous = [...subMap.values()].some(c => c > 1);

// //   const durs      = notes.map(n => n.duration_subdivisions);
// //   const durString = durs.join('-');
// //   const isUniform = durs.every(d => d === durs[0]);
// //   const uniformDur = isUniform ? durs[0] : null;
// //   const maxDur    = Math.max(...durs);
// //   const minDur    = Math.min(...durs);

// //   let rhythmClass;
// //   if      (isUniform && uniformDur === 1)                             rhythmClass = 'rapid';
// //   else if (isUniform && uniformDur === 2 && !hasSimultaneous)         rhythmClass = 'weave';
// //   else if (isUniform && uniformDur === 2 && hasSimultaneous)          rhythmClass = 'interval';
// //   else if (notes.length <= 3 && maxDur >= Math.round(SPB / 2))       rhythmClass = 'sustain';
// //   else if (notes.length <= 6 && maxDur >= Math.round(SPB / 2))       rhythmClass = 'lyrical';
// //   else if (isUniform)                                                  rhythmClass = 'pulse';
// //   else                                                                 rhythmClass = 'mixed';

// //   const sim         = hasSimultaneous ? '1' : '0';
// //   const fingerprint = `${notes.length}|${durString}|${sim}`;

// //   return {
// //     noteCount: notes.length, durString, isUniform, uniformDur,
// //     maxDur, minDur, hasSimultaneous, isEmpty: false,
// //     rhythmClass, fingerprint,
// //   };
// // }

// // // Human-readable description of a motif
// // function describeMotif(fp) {
// //   if (fp.isEmpty) return 'Silence';

// //   const durPart = fp.isUniform
// //     ? `d=${fp.uniformDur} (${fp.uniformDur === 1 ? '16th' : fp.uniformDur === 2 ? '8th' : fp.uniformDur === 4 ? 'quarter' : fp.uniformDur + ' sub'})`
// //     : `d=${fp.minDur}–${fp.maxDur} (mixed)`;

// //   const texturePart = {
// //     'rapid':    'rapid 16th-note figure',
// //     'weave':    'alternating 8th-note weave',
// //     'interval': 'simultaneous interval pairs',
// //     'sustain':  'sustained long tones',
// //     'lyrical':  'lyrical sparse phrase',
// //     'pulse':    'uniform pulse figure',
// //     'mixed':    'varied rhythm phrase',
// //   }[fp.rhythmClass] || fp.rhythmClass;

// //   const simNote = fp.hasSimultaneous ? ' · simultaneous pairs' : '';
// //   return `${fp.noteCount} notes/bar · ${durPart}${simNote} · ${texturePart}`;
// // }


// // // ─── UTILITIES ──────────────────────────────────────────────────────

// // // Compress a sorted list of bar numbers into ranges: [3,4,5,7,8] → "3–5, 7–8"
...
// //   '#a78bfa', // J — violet
// // ];
// // const COLOR_S = '#374151'; // silence (dark gray)
// // const COLOR_X = '#6b7280'; // unique (gray)
...
// // // ═══════════════════════════════════════════════════════════════════
// // // MOTIF DETECTION — core DSA function
// // //
// // // Steps:
// // //   1. fingerprint every bar
// // //   2. count frequency of each fingerprint (skip empty)
// // //   3. rank fingerprints by frequency → assign labels A, B, C...
// // //      (most bars = A, next = B, etc.)
// // //   4. fingerprints with only 1 occurrence → label X (unique)
// // //   5. build per-motif detail objects
// // // ═══════════════════════════════════════════════════════════════════

// // function findMotifs(bars, spb) {
// //   const fps = bars.map(b => barFingerprint(b, spb));

// //   // Count fingerprint frequency (skip empty)
// //   const freq = new Map();
// //   for (const fp of fps) {
// //     if (!fp.isEmpty) freq.set(fp.fingerprint, (freq.get(fp.fingerprint) || 0) + 1);
// //   }

// //   // Sort by frequency descending; keep only motifs (2+ occurrences)
// //   const ranked = [...freq.entries()]
// //     .filter(([, c]) => c >= 2)
// //     .sort((a, b) => b[1] - a[1]);

// //   // Assign label A, B, C... to ranked fingerprints
// //   const LABELS = 'ABCDEFGHIJKLMNOP'.split('');
// //   const motifMap = new Map(); // fingerprint string → label char
// //   ranked.forEach(([fp], i) => {
// //     if (i < LABELS.length) motifMap.set(fp, LABELS[i]);
// //   });

// //   // Label each bar
// //   const labeled = bars.map((bar, i) => {
// //     const fp    = fps[i];
// //     const label = fp.isEmpty ? 'S' : (motifMap.get(fp.fingerprint) ?? 'X');
// //     return { bar, fp, label };
// //   });

// //   // Build motif detail objects per label
// //   const motifDetails = new Map(); // label → detail
...
// //   // Add descriptions and compressed bar ranges
// //   for (const [, d] of motifDetails) {
// //     d.description = describeMotif(d.fp);
// //     d.barRanges   = compressRanges(d.barNumbers);
// //   }

// //   // Sort motif details: A < B < C ... < X (X always last)
...
// // //
// // // Algorithm:
// // //   1. RLE compress the label sequence → list of {label, startBar, endBar, len}
// // //   2. Find primary motif = label covering the most total bars
// // //   3. Absorb "short" runs (≤ 2 bars, non-S) into the adjacent major run
// // //      → this merges brief transitions/breaths into their parent section
// // //   4. Merge consecutive same-dominant runs into one section
// // //   5. Classify section type:
// // //      Silence / Exposition / Development / Reprise / Coda
// // // ═══════════════════════════════════════════════════════════════════
...
// //     runs.push({
// //       label:    lbl,
// //       startBar: labeled[i].bar.bar_number,
// //       endBar:   labeled[j - 1].bar.bar_number,
// //       len:      j - i,
// //     });
// //     i = j;
// //   }
...
// //   // Find primary motif: label covering most bars (excluding S and X)
// //   const barCoverage = new Map();
// //   for (const r of runs) {
// //     if (r.label !== 'S' && r.label !== 'X')
// //       barCoverage.set(r.label, (barCoverage.get(r.label) || 0) + r.len);
// //   }
// //   let primaryLabel = null, primaryCount = 0;
// //   for (const [l, c] of barCoverage) {
// //     if (c > primaryCount) { primaryLabel = l; primaryCount = c; }
// //   }

// //   // Absorb short runs (≤2 bars, non-S) into their previous or next major run
// //   // This merges brief "breath" bars into the surrounding section
// //   const merged = [];
// //   for (let i = 0; i < runs.length; i++) {
// //     const run = runs[i];
// //     // Short non-S run → try to absorb into previous
// //     if (run.label !== 'S' && run.len <= 2 && merged.length > 0) {
// //       const prev = merged[merged.length - 1];
// //       // Only absorb if previous is not S and adjacent
// //       if (prev.label !== 'S') {
// //         prev.endBar = run.endBar;
// //         prev.len   += run.len;
// //         if (!prev.absorbed) prev.absorbed = [];
// //         prev.absorbed.push(run.label);
// //         continue;
// //       }
// //     }
// //     merged.push({ ...run, absorbed: [] });
// //   }

// //   // Merge consecutive same-dominant runs into one section
// //   const sections = [];
// //   let mi = 0;
...
// //     const secMerged = [cur];
// //     let mj = mi + 1;
// //     // Merge: same label, non-S
// //     while (mj < merged.length && merged[mj].label === cur.label && cur.label !== 'S') {
// //       secMerged.push(merged[mj]);
// //       mj++;
// //     }
// //     const allLabels = [...new Set(secMerged.flatMap(r => [r.label, ...r.absorbed]))].filter(Boolean);
// //     sections.push({
// //       startBar:      secMerged[0].startBar,
// //       endBar:        secMerged[secMerged.length - 1].endBar,
// //       dominantLabel: cur.label,
// //       labels:        allLabels,
// //     });
// //     mi = mj;
// //   }
...
// //   const ROMAN = ['I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII','XIII','XIV','XV','XVI'];
// //   const primarySections = sections.filter(s => s.dominantLabel === primaryLabel);

// //   return sections.map((s, idx) => {
// //     let type;
// //     if (s.dominantLabel === 'S') {
// //       type = 'Silence';
// //     } else if (s.dominantLabel === primaryLabel) {
// //       const isFirst = primarySections[0] === s;
// //       const isLast  = primarySections[primarySections.length - 1] === s;
// //       if (isFirst)         type = 'Exposition';
// //       else if (isLast)     type = 'Coda';
// //       else                 type = 'Reprise';
// //     } else {
// //       type = 'Development';
// //     }

// //     const barRange = s.startBar === s.endBar
// //       ? `${s.startBar}`
// //       : `${s.startBar}–${s.endBar}`;

// //     const motifStr = [...new Set(s.labels)].filter(l => l !== 'S' && l !== 'X').join(' + ') || 'S';
...
// //       endBar:        s.endBar,
// //       barRange,
// //       dominantLabel: s.dominantLabel,
// //       labels:        s.labels,
// //       motifStr,
// //       color:         motifColor(s.dominantLabel),
// //     };
// //   });
// // }
...
// // // ═══════════════════════════════════════════════════════════════════
// // // LEFT HAND — chord cycle detection
// // // ═══════════════════════════════════════════════════════════════════
...
// //   return chordName(root, midis);
// // }

// // function detectChordCycle(bars) {
// //   // Build chord sequence from non-empty bars
// //   const chords = bars
// //     .map(b => lhChordForBar(b))
// //     .filter(c => c !== null);

// //   if (chords.length < 4) return null;

// //   // Try cycle periods 1 to 16
// //   for (let period = 1; period <= 16; period++) {
// //     const cycle    = chords.slice(0, period);
// //     const cycleStr = cycle.join(',');
// //     let reps = 1;
// //     while ((reps + 1) * period <= chords.length) {
// //       const slice = chords.slice(reps * period, (reps + 1) * period);
// //       if (slice.join(',') === cycleStr) reps++;
// //       else break;
// //     }
// //     // Need at least 2 complete repetitions to call it a cycle
// //     if (reps >= 2) {
// //       return {
// //         cycle,
...
// //   const sections = detectSections(labeled);

// //   // Build per-bar info for frontend
// //   // Mark boundary-end bars (last bar of each section)
// //   const boundaryEndBars = new Set(sections.map(s => s.endBar));
...
// //   const labeledBars = labeled.map(({ bar, fp, label }) => {
// //     const motif = motifDetails.get(label);
...
// //       role:         label === 'S' ? 'silence' : label === 'X' ? 'unique' : 'primary',
...
// //       sectionLabel: sec?.romanLabel ?? null,
// //       sectionType:  sec?.type ?? null,
// //       isBoundaryEnd: boundaryEndBars.has(bar.bar_number),
// //       isUnique:     label === 'X',
// //     };
// //   });

// //   return { motifs: sortedMotifs, labeled: labeledBars, sections };
// // }

// // function emptyTrackResult() {
// //   return { motifs: [], labeled: [], sections: [] };
// // }


// // // ═══════════════════════════════════════════════════════════════════
// // // YAML GENERATOR — clean, minimal output
// // // ═══════════════════════════════════════════════════════════════════

// // function generateYaml(rhResult, lhResult, lhChordCycle, metadata) {
// //   const totalBars = metadata.bars.length;
// //   const header    = `# COMPOSITION ANALYSIS\n# ${totalBars} bars · ${metadata.time_signature} · ${metadata.tempo} BPM · Key: ${metadata.key}`;

// //   // Right hand motifs
// //   const rhMotifLines = (rhResult.motifs || [])
// //     .filter(m => m.label !== 'X')
// //     .map(m =>
// //       `    ${m.label}:\n` +
// //       `      count: ${m.count}\n` +
...
// //   // Right hand sections
// //   const rhSecLines = (rhResult.sections || []).map(s =>
// //     `    ${s.romanLabel}: {bars: "${s.barRange}", type: ${s.type}, motifs: [${s.motifStr}]}`
// //   ).join('\n');

// //   // Left hand motifs
// //   const lhMotifLines = (lhResult?.motifs || [])
// //     .filter(m => m.label !== 'X')
// //     .map(m =>
// //       `    ${m.label}:\n` +
// //       `      count: ${m.count}\n` +
...
// //   // Left hand sections
// //   const lhSecLines = (lhResult?.sections || []).map(s =>
// //     `    ${s.romanLabel}: {bars: "${s.barRange}", type: ${s.type}, motifs: [${s.motifStr}]}`
// //   ).join('\n');

// //   // Chord cycle
// //   const chordLine = lhChordCycle
// //     ? `  chord_cycle: "${lhChordCycle.cycleString} (${lhChordCycle.periodBars}-bar cycle × ${lhChordCycle.repeatCount})"`
// //     : '  chord_cycle: n/a';

// //   const hasRh = (rhResult.motifs || []).length > 0;
// //   const hasLh = (lhResult?.motifs || []).length > 0;
...
// //     if (input.rh) {
// //       const norm = normalizeJson(input.rh);
// //       metadata = metadata ?? norm; spb = norm.subdivisions_per_bar; rhBars = norm.bars;
// //     }
// //     if (input.lh) {
// //       const norm = normalizeJson(input.lh);
// //       metadata = metadata ?? norm; lhBars = norm.bars;
// //     }
// //     if (!metadata && (rhBars || lhBars))
// //       metadata = { tempo:120, time_signature:'4/4', key:'C', subdivisions_per_bar:16, bars: rhBars || lhBars };
// //   }
...
// //   const rhResult = rhBars ? analyzeTrack(rhBars, spb) : emptyTrackResult();
// //   const lhResult = lhBars ? analyzeTrack(lhBars, spb) : emptyTrackResult();

// //   // Chord cycle detection on raw LH bars
// //   const lhChordCycle = lhBars ? detectChordCycle(lhBars) : null;

// //   const yaml = generateYaml(rhResult, lhResult, lhChordCycle, metadata);

// //   const splitDisplay = splitMidi
// //     ?? (rhBars && lhBars ? detectSplitMidi([...rhBars, ...lhBars]) : null);

// //   return {
// //     metadata,
// //     splitMidi: splitDisplay,
// //     rightHand: {
// //       motifs:   rhResult.motifs,
// //       labeled:  rhResult.labeled,
// //       sections: rhResult.sections,
// //     },
// //     leftHand: {
// //       motifs:     lhResult.motifs,
// //       labeled:    lhResult.labeled,
// //       sections:   lhResult.sections,
// //       chordCycle: lhChordCycle,
// //     },
// //     yamlBlueprint: yaml,
// //     summary: {
// //       totalBars:    metadata.bars.length,
...
// //       splitMidi:    splitDisplay,
// //       chordCycle:   lhChordCycle?.cycleString ?? null,
// //     },
// //   };
// // }
...
// //   splitHandsFromFull, detectSplitMidi,
// //   pitchToMidi, midiToName, chordName,
// //   rleRuns,
// // };
...
// // ═══════════════════════════════════════════════════════════════════
// // MUSIC ANALYZER ENGINE v8.1 — MOTIF-FIRST DSA ENGINE
// //
// // Core algorithm — no clustering, no thresholds, no pair-windows:
// //
// //   STEP 1  quantizeDuration(d, spb)
// //     Snap raw subdivision counts to musical-value buckets:
// //     a=16th  b=8th  c=quarter-ish  d=half-ish  e=dotted-half  f=whole  g=multi-bar
// //     Handles ±1-2 sub quantization noise → same feel = same bucket.
// //
// //   STEP 2  barFingerprint(bar, spb)
// //     fingerprint = "noteCount|quantDurs|simultaneous"
// //     Pitch-agnostic. Same rhythm at any key = same fingerprint.
// //     Rests / silences that are continuations of a previous note
// //     are detected (markContinuationBars) and given the parent label.
// //
// //   STEP 3  findMotifs(bars, spb)
// //     Count fingerprint frequencies.
// //     Frequency-ranked → labels A, B, C, …
// //     Fingerprint with only 1 occurrence → label X (unique).
// //     Continuation bars inherit the label of the bar whose note spans into them.
// //
// //   STEP 4  mergePairedMotifs(labeled, motifDetails)
// //     If motif Y almost always (≥ 90 %) follows motif X AND
// //     Y almost always (≥ 90 %) appears only after X
// //     → Y is merged into X as a 2-bar unit.
// //     Example: Passacaglia B-first-bar always followed by B-second-bar
// //     → they become one 2-bar Motif B instead of two separate motifs.
// //
// //   STEP 5  detectSections(labeled)
// //     RLE compress, absorb micro-transitions, find inner repeating cycle
// //     (detectRepeat) to label sections as "A(6)B(2) × 5".
// //
// // Works for any style. Deterministic. No floating-point thresholds.
// // ═══════════════════════════════════════════════════════════════════
...
// // ═══════════════════════════════════════════════════════════════════
// // STEP 1: DURATION QUANTIZATION
// //
// // Snap raw subdivision counts to musical-value buckets.
// // This gives tolerance against ±1-2 sub quantization noise.
// //
// // Buckets relative to one bar (SPB subdivisions):
// //   a = ≤ 1 sub           (16th note or shorter)
// //   b = ≤ 2 subs          (8th note)
// //   c = ≤ ~31% of bar     (quarter-ish, 3-5 subs for SPB=16)
// //   d = ≤ 50% of bar      (half-ish, 6-8 subs for SPB=16)
// //   e = ≤ 75% of bar      (dotted-half-ish, 9-12 subs)
// //   f = ≤ 100% of bar     (whole note, 13-16 subs)
// //   g = > bar             (multi-bar sustain)
// // ═══════════════════════════════════════════════════════════════════
...
//   if (d <= 1)                      return 'a';
//   if (d <= 2)                      return 'b';
//   if (d <= Math.ceil(SPB * 0.31))  return 'c'; // 3-5 for SPB=16
//   if (d <= Math.ceil(SPB * 0.50))  return 'd'; // 6-8 for SPB=16
//   if (d <= Math.ceil(SPB * 0.75))  return 'e'; // 9-12 for SPB=16
//   if (d <= SPB)                    return 'f'; // 13-16 for SPB=16
//   return 'g'; // cross-bar sustain
// }

// // Human-readable duration name for descriptions
// function durBucketName(bucket) {
//   const names = { a:'16th', b:'8th', c:'quarter', d:'half', e:'dotted-half', f:'whole', g:'multi-bar' };
//   return names[bucket] ?? bucket;
// }


// // ═══════════════════════════════════════════════════════════════════
// // STEP 2: BAR FINGERPRINT
// //
// // fingerprint = "noteCount | quantizedDurString | simultaneous"
// //
// // Pitch-agnostic (same rhythm at any register = same fingerprint).
// // Simultaneous flag ('1'/'0') distinguishes chordal from melodic.
// // ═══════════════════════════════════════════════════════════════════
...
//   if (!notes.length) {
//     return {
//       noteCount: 0, quantDurStr: '', isUniform: true, uniformQ: null,
//       maxDur: 0, minDur: 0, hasSimultaneous: false, isEmpty: true,
//       rhythmClass: 'empty', fingerprint: 'EMPTY',
//     };
//   }
...
//   // Classify overall rhythmic texture
//   let rhythmClass;
...
//   else if (notes.length <= 5 && (uniformQ === 'e' || uniformQ === 'f' || uniformQ === 'g')) rhythmClass = 'sustain';
//   else if (notes.length <= 6 && maxDur >= Math.round(SPB / 2))       rhythmClass = 'lyrical';
//   else if (isUniform)                                                  rhythmClass = 'pulse';
//   else                                                                 rhythmClass = 'mixed';
...
//   return {
//     noteCount: notes.length, quantDurStr, isUniform, uniformQ,
//     maxDur, minDur, hasSimultaneous, isEmpty: false,
//     rhythmClass, fingerprint,
//   };
// }

// // Plain-language description of a motif's texture
...
//   const texturePart = {
//     'rapid':    'rapid 16th-note run',
//     'weave':    'alternating 8th-note weave',
//     'interval': 'simultaneous interval pairs',
//     'sustain':  'sustained long tone',
//     'lyrical':  'lyrical sparse phrase',
//     'pulse':    'steady pulse figure',
//     'mixed':    'varied rhythm phrase',
//   }[fp.rhythmClass] || fp.rhythmClass;

//   const simNote  = fp.hasSimultaneous ? ' · simultaneous pairs' : '';
//   const unitNote = unitBars > 1 ? ` · ${unitBars}-bar unit` : '';
//   return `${fp.noteCount} notes/bar · ${durPart}${simNote} · ${texturePart}${unitNote}`;
// }


// // ─── UTILITIES ──────────────────────────────────────────────────────

// // Compress sorted bar-number array into range strings: [3,4,5,7,8] → "3–5, 7–8"
// function compressRanges(barNumbers) {
//   if (!barNumbers.length) return '';
//   const sorted = [...new Set(barNumbers)].sort((a, b) => a - b);
//   const ranges = [];
...
//   '#f97316','#a78bfa',
// ];
// const COLOR_S = '#374151';
// const COLOR_X = '#6b7280';
...
// //
// // In the compact JSON format, a note that spans multiple bars is
// // declared ONCE in its starting bar with d > SPB.
// // The subsequent bars therefore appear EMPTY in the JSON but are
// // musically filled by the sustained note.
// //
// // We mark such bars as "continuation" so findMotifs can give them
// // the same label as the bar whose note spans into them, rather than
// // the 'S' (silence) label.
// // ═══════════════════════════════════════════════════════════════════
...
//           const targetBn = bar.bar_number + i;
//           if (!continuations.has(targetBn))
//             continuations.set(targetBn, bar.bar_number);
//         }
//       }
//     }
//   }
//   return continuations; // Map<barNumber, sourcBarNumber>
// }
...
// //
// // 1. Fingerprint every bar (using quantized durations).
// // 2. Continuation bars (sustained notes) don't add to frequency count.
// // 3. Count fingerprint frequency; rank by frequency → A, B, C…
// //    (1-occurrence fingerprints → X).
// // 4. Label each bar; continuation bars inherit parent label.
// // ═══════════════════════════════════════════════════════════════════
...
//   // Count frequency — skip empty and continuation bars
//   const freq = new Map();
//   bars.forEach((bar, i) => {
//     const fp = fps[i];
//     if (fp.isEmpty) return;
//     if (continuations.has(bar.bar_number)) return; // skip continuations
//     freq.set(fp.fingerprint, (freq.get(fp.fingerprint) || 0) + 1);
//   });

//   // Sort by frequency; keep only motifs (≥2 occurrences)
//   const ranked = [...freq.entries()]
//     .filter(([, c]) => c >= 2)
//     .sort((a, b) => b[1] - a[1]);

//   const LABELS = 'ABCDEFGHIJKLMNOP'.split('');
//   const motifMap = new Map(); // fingerprint → label
//   ranked.forEach(([fp], i) => {
//     if (i < LABELS.length) motifMap.set(fp, LABELS[i]);
//   });
...
//     const fp = fps[i];
//     if (continuations.has(bar.bar_number)) return null; // placeholder, resolved next
//     const label = fp.isEmpty ? 'S' : (motifMap.get(fp.fingerprint) ?? 'X');
...
//     const srcEntry = srcIdx !== undefined ? labelArr[srcIdx] : null;
//     const parentLabel = srcEntry?.label ?? 'S';
//     return {
//       bar, fp: fps[i], label: parentLabel,
//       isContinuation: true,
//     };
//   });
...
//   labeled.forEach(({ bar, fp, label, isContinuation }) => {
//     if (label === 'S') return;
//     if (!motifDetails.has(label)) {
//       motifDetails.set(label, {
//         label, fingerprint: fp.fingerprint, fp,
//         barNumbers: [], count: 0,
//         description: '', barRanges: '',
//         color: motifColor(label),
//         unitBars: 1,
//       });
//     }
...
// // ═══════════════════════════════════════════════════════════════════
// // STEP 4: MOTIF PAIR MERGING
// //
// // Detect when motif Y is almost exclusively a "second bar" that
// // always follows motif X (≥ PAIR_THRESHOLD confidence both ways).
// // If so, merge Y into X as a 2-bar unit.
// //
// // Example: Passacaglia Pattern B spans 2 bars:
// //   bar1 = chromatic sigh (5 notes, mixed)   → Motif B
// //   bar2 = long held note (1-2 notes, long)  → Motif C
// //   B always→C (conf=1.0), C always after B (conf=1.0)
// //   → C is merged into B, resulting in one 2-bar Motif B.
// // ═══════════════════════════════════════════════════════════════════

// const PAIR_THRESHOLD = 0.90;

// function mergePairedMotifs(labeled, motifDetails) {
//   // Count transitions X → Y
//   const forwardTrans = new Map(); // label → Map<nextLabel, count>
//   const totalFrom    = new Map(); // label → total transitions out
//   const totalTo      = new Map(); // label → total times appearing as target

//   for (let i = 0; i < labeled.length - 1; i++) {
//     const from = labeled[i].label;
//     const to   = labeled[i + 1].label;
...
//   // Find merge candidates: Y merges into X if:
//   //   P(Y | X) ≥ PAIR_THRESHOLD  (X almost always leads to Y)
//   //   P(Y-after-X | Y) ≥ PAIR_THRESHOLD  (Y almost always appears after X)
//   const mergeInto = new Map(); // child label → parent label

//   for (const [from, toMap] of forwardTrans) {
//     const outTotal = totalFrom.get(from) || 0;
//     for (const [to, count] of toMap) {
//       if (to === from) continue; // skip self-loops
//       const pForward = count / outTotal; // P(to | from)
//       const inTotal  = totalTo.get(to) || 0;
//       const pBack    = inTotal > 0 ? count / inTotal : 0; // P(to after from | to appears)

//       if (pForward >= PAIR_THRESHOLD && pBack >= PAIR_THRESHOLD) {
//         // 'to' is almost exclusively a continuation of 'from'
//         // But make sure 'from' doesn't already have a parent (no chain merging)
//         if (!mergeInto.has(from)) {
...
//   // Apply merge: relabel child bars to parent label
...
//         ...entry,
//         label: parentLabel,
//         color: parentDet?.color ?? entry.color,
//         patternLabel: `Motif ${parentLabel}`,
...
//   // Update motif details: absorb child into parent
//   const newDetails = new Map(
//     [...motifDetails.entries()].map(([k, v]) => [k, { ...v, barNumbers: [...v.barNumbers] }])
//   );

//   for (const [childLabel, parentLabel] of mergeInto) {
//     const child  = newDetails.get(childLabel);
//     const parent = newDetails.get(parentLabel);
//     if (!child || !parent) continue;

//     // Merge child bars into parent
...
//     parent.unitBars   = 2; // now a 2-bar unit
//     parent.description = describeMotif(parent.fp, 2);

//     // Remove the child motif
...
// // ═══════════════════════════════════════════════════════════════════
// // DETECT REPEATING PATTERN WITHIN A LABEL SEQUENCE
// //
// // Finds the smallest period P such that arr[i] === arr[i % P] for
// // all i. If found, returns { pattern, period, reps, cycleStr }.
// //
// // Example: [A,A,A,A,A,A,B,B,A,A,A,A,A,A,B,B,...] → P=8, reps=5
// // Example: [A,B,A,B,A,B,A,B] → P=2, reps=4
// // ═══════════════════════════════════════════════════════════════════
...
//     // Check if arr has period P
//     let matches = true;
//     for (let i = P; i < n; i++) {
//       if (arr[i] !== arr[i % P]) { matches = false; break; }
//     }
//     if (!matches) continue;
...
//     // Compress pattern to RLE description: A,A,A,A,A,A,B,B → A(6)B(2)
//     const pRle  = [];
//     let pi = 0;
//     while (pi < P) {
//       const lbl = pattern[pi];
//       let pj = pi;
//       while (pj < P && pattern[pj] === lbl) pj++;
//       pRle.push(pj - pi === 1 ? lbl : `${lbl}(${pj - pi})`);
//       pi = pj;
//     }
//     const cycleStr = `${pRle.join('')}×${reps}`;

//     return { pattern, period: P, reps, cycleStr };
//   }
//   return null;
// }
...
// //
// // 1. RLE compress the label sequence.
// // 2. Short non-silence runs (≤ 2 bars) get absorbed into adjacent
// //    major run — this prevents micro-breaks from creating tiny sections.
// // 3. Consecutive runs of the same dominant label are merged.
// // 4. Each section gets: romanLabel, type, barRange, motifStr, cycleStr.
// // 5. detectRepeat is run on the label sequence within each section
// //    to produce the inner cycle description (e.g., "A(6)B(2)×5").
// // ═══════════════════════════════════════════════════════════════════
...
//   // Find primary motif (most total bars, excluding S and X)
//   const barCoverage = new Map();
//   for (const r of runs) {
//     if (r.label !== 'S' && r.label !== 'X')
//       barCoverage.set(r.label, (barCoverage.get(r.label) || 0) + r.len);
...
//   // Absorb short runs (≤ 2 bars, non-S) into previous major run
//   const merged = [];
//   for (let i = 0; i < runs.length; i++) {
//     const run = runs[i];
//     if (run.label !== 'S' && run.len <= 2 && merged.length > 0 && merged[merged.length - 1].label !== 'S') {
//       const prev = merged[merged.length - 1];
//       prev.endBar = run.endBar;
//       prev.len   += run.len;
//       if (!prev.subLabels) prev.subLabels = [prev.label];
//       prev.subLabels.push(run.label);
//     } else {
//       merged.push({ ...run, subLabels: [run.label] });
//     }
//   }

//   // Merge consecutive same-dominant runs (non-S)
//   const sections = [];
//   let mi = 0;
//   while (mi < merged.length) {
//     const cur      = merged[mi];
...
//     let mj = mi + 1;
//     while (mj < merged.length && merged[mj].label === cur.label && cur.label !== 'S') {
//       secRuns.push(merged[mj]);
//       mj++;
//     }
//     const allSubs = [...new Set(secRuns.flatMap(r => r.subLabels || [r.label]))];
//     sections.push({
//       startBar:      secRuns[0].startBar,
//       endBar:        secRuns[secRuns.length - 1].endBar,
//       dominantLabel: cur.label,
//       labels:        allSubs,
//     });
//     mi = mj;
...
//   // For each section, find the inner repeating cycle
//   const barToLabel = new Map(labeled.map(lb => [lb.bar.bar_number, lb.label]));
//   const ROMAN = ['I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII','XIII','XIV','XV','XVI'];
//   const primarySections = sections.filter(s => s.dominantLabel === primaryLabel);

//   return sections.map((s, idx) => {
//     // Build the label sequence for this section's bar range
//     const secLabels = [];
//     for (let b = s.startBar; b <= s.endBar; b++) {
//       const lbl = barToLabel.get(b) ?? s.dominantLabel;
//       secLabels.push(lbl);
//     }

//     const innerCycle = s.dominantLabel !== 'S' ? detectRepeat(secLabels) : null;
//     const cycleStr   = innerCycle ? innerCycle.cycleStr : null;
...
//     const barRange  = s.startBar === s.endBar ? `${s.startBar}` : `${s.startBar}–${s.endBar}`;
//     const uniqueMotifs = [...new Set(s.labels)].filter(l => l !== 'S' && l !== 'X');
//     const motifStr  = uniqueMotifs.join(' + ') || 'S';
...
//   // Steps 1-3: fingerprint + label
//   const { labeled, sortedMotifs, motifDetails } = findMotifs(bars, spb);

//   // Step 4: merge paired motifs (detect 2-bar units)
//   const merged = mergePairedMotifs(labeled, motifDetails);
//   const finalLabeled   = merged.labeled;
//   const finalDetails   = merged.motifDetails;
//   const finalMotifs    = merged.sortedMotifs;

//   // Step 5: sections
//   const sections = detectSections(finalLabeled);

//   // Boundary bars (last bar of each section)
//   const boundaryEndBars = new Set(sections.map(s => s.endBar));

//   // Section lookup by bar number
//   const secByBar = new Map();
//   for (const sec of sections)
//     for (let b = sec.startBar; b <= sec.endBar; b++)
//       secByBar.set(b, sec);

//   // Build per-bar labeled list for frontend
//   const labeledBars = finalLabeled.map(({ bar, fp, label, isContinuation, isMergedContinuation }) => {
//     const motif = finalDetails.get(label);
//     const sec   = secByBar.get(bar.bar_number);
//     return {
//       barNumber:    bar.bar_number,
//       notes:        bar.notes,
...
//       patternLabel: label === 'S' ? 'Silence' : label === 'X' ? 'Unique' : `Motif ${label}`,
//       patternId:    `MOT_${label}`,
//       role:         label === 'S' ? 'silence' : label === 'X' ? 'unique' : (isContinuation || isMergedContinuation) ? 'continuation' : 'primary',
//       color:        motif?.color ?? motifColor(label),
//       isEmpty:      fp.isEmpty,
//       noteCount:    bar.notes.length,
//       rhythmClass:  fp.rhythmClass,
...
//   return { motifs: finalMotifs, labeled: labeledBars, sections };
// }
...
//     let reps = 1;
//     while ((reps + 1) * period <= chords.length) {
//       if (chords.slice(reps * period, (reps + 1) * period).join(',') === cycleStr) reps++;
//       else break;
//     }
//     if (reps >= 2) {
//       return { cycle, cycleString: cycle.join(' → '), periodBars: period, repeatCount: reps };
//     }
//   }
//   return null;
// }
...
//       .filter(m => m.label !== 'X')
//       .map(m => {
//         const unitNote = m.unitBars > 1 ? `\n      unit_bars: ${m.unitBars}` : '';
//         return (
//           `    ${m.label}:\n` +
//           `      count:  ${m.count}${unitNote}\n` +
//           `      bars:   "${m.barRanges}"\n` +
//           `      descr:  "${m.description}"`
//         );
//       })
...
//     (sections || []).map(s => {
//       const cycle = s.cycleStr ? `, cycle: ${s.cycleStr}` : '';
//       return `    ${s.romanLabel}: {bars: "${s.barRange}", type: ${s.type}, motifs: [${s.motifStr}]${cycle}}`;
//     }).join('\n');

//   const rhM = motifLines(rhResult.motifs);
//   const rhS = sectionLines(rhResult.sections);
//   const lhM = motifLines(lhResult?.motifs);
//   const lhS = sectionLines(lhResult?.sections);
...
//   detectSections, detectRepeat, barFingerprint,
//   quantizeDuration, describeMotif, compressRanges,
