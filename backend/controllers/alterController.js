...
async function alter(req, res, next) {
  try {
    ...
    // Pick service based on model param  ('aria' = Gemini, 'opus' = Claude)
    ...
    const merged = {
      ...
      bars: original.bars.map(origBar => {
        const addBar   = (additionsJson.bars || []).find(b => b.bar_number === origBar.bar_number)
        const addNotes = addBar?.notes || []
        return { bar_number: origBar.bar_number, notes: [...(origBar.notes || []), ...addNotes] }
      }),
    }
    ...
    res.json({
      ...
      bars,
      addedNotes,
      model:        modelLabel,    // ← returned so frontend knows which was used
      originalFile: req.file.originalname,
    })
  } catch (err) {
    next(err)
