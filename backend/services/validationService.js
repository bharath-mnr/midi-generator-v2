...
function validate(json) {
  ...
  for (const bar of json.bars) {
    ...
    for (const note of bar.notes) {
      ...
      // note does not extend beyond end of piece — clamp instead of error
      if (subs) {
        ...
        if (endAbs > maxAbs) {
          // Auto-clamp: shorten the note to fit rather than rejecting
          note.duration_subdivisions = Math.max(1, maxAbs - startAbs)
        }
      }
