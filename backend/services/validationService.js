...
'use strict'
// backend/services/validationService.js
// Validates doc objects produced by parseCompactString().
// Accepts both bar_number (new) and bn (old) field names for backward compat.
// velocity is never validated — always 100, never from AI.
...
// Normalise note — handles compact (p/s/d) and full field names
function norm(n) {
  return {
    pitch:                 n.pitch                 ?? n.p,
    start_subdivision:     n.start_subdivision     ?? n.s   ?? 0,
...
// Normalise bar — handles bar_number (new) and bn (old)
...
function validate(json) {
  ...
  for (const bar of json.bars) {
    ...
    for (const rawNote of bar.notes) {
      ...
      if (!note.pitch || !String(note.pitch).match(/^[A-G][#b]?-?\d+$/i)) {
        errors.push(`${loc}: invalid pitch format "${note.pitch}"`)
      }
      if (typeof note.start_subdivision !== 'number'
          || note.start_subdivision < 0
      ...
      if (subs) {
        ...
        const maxAbs   = json.bars.length * subs
        if (endAbs > maxAbs) {
          const clamped = Math.max(1, maxAbs - startAbs)
          rawNote.d = rawNote.duration_subdivisions = clamped
        }
      }
  ...
  // Bar number continuity — non-fatal warning for large gaps
  const barNums = json.bars.map(b => getBarNum(b)).filter(n => typeof n === 'number').sort((a, b) => a - b)
  for (let i = 1; i < barNums.length; i++) {
    if (barNums[i] - barNums[i - 1] > 8) {
      errors.push(`Large gap in bar numbers: ${barNums[i - 1]} → ${barNums[i]}`)
    }
...
// Validate a raw compact string — parse first, then validate the doc
function validateCompactString(str) {
  try {
    const { parseCompactString } = require('./converters/stringToMidi')
    const doc = parseCompactString(str)
    return validate(doc)
  } catch (err) {
    return { ok: false, errors: [`Compact string parse error: ${err.message}`] }
  }
}
...
module.exports = { validate, validateCompactString, summarise }