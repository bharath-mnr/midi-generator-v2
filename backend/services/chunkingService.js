...
'use strict'
// backend/services/chunkingService.js  v4.0
//
// ── WHAT CHANGED ──────────────────────────────────────────────────────────────
//  buildNotePatternSample() now outputs COMPACT STRING format instead of JSON.
//  This reduces note_pattern chunk tokens by ~80% — the most expensive chunk type.
//
//  Example (old JSON):
//    [{"bn":1,"notes":[{"p":"D2","s":0,"d":16},{"p":"F4","s":4,"d":4},...]}]
//
//  Example (new compact string):
//    B1: D2,0,16; F4,4,4; A4,8,8
//    B2: D2,0,16; A4,0,8
//
//  Everything else (metadata, style, structure chunks) unchanged.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Note field accessors (handles compact and full format) ─────────────────────
function getPitch(n)  { return n.pitch                ?? n.p               }
function getDur(n)    { return n.duration_subdivisions ?? n.d ?? 4         }
function getStart(n)  { return n.start_subdivision    ?? n.s ?? 0          }
function getOffset(n) { return n.offset_percent       ?? n.o ?? 0          }
...
function extractPieceName(filename) {
  return filename
    .replace(/\.[^.]+$/, '')
    .replace(/[_-]*(rh|right_hand|righthand|right)$/i, '')
    .replace(/[_-]*(lh|left_hand|lefthand|left)$/i,   '')
    .replace(/[_-]*(full_track|full)$/i,               '')
    .replace(/[-_]+/g, ' ')
    .trim()
}
...
function computeStats(notes) {
  ...
  const avgDur     = notes.reduce((s, n) => s + getDur(n), 0) / notes.length
  return {
    low:         midiToName(Math.min(...midis)),
    high:        midiToName(Math.max(...midis)),
    avgDur:      avgDur.toFixed(1),
    count:       notes.length,
    articulation: describeArticulation(avgDur),
    hasSwing:    notes.some(n => getOffset(n) > 0),
    hasStaccato: notes.some(n => getCutoff(n) != null),
  }
}
...
function describeStructurePattern(bars) {
  ...
  const peaks = densities.filter(d => d > avg * 1.8).length
  const parts = [`avg ${avg.toFixed(1)} notes/bar`]
  if (zeros > 0) parts.push(`${zeros} empty bars`)
  if (peaks > 0) parts.push(`${peaks} climax bars`)
  if (max > avg * 2.5) parts.push(`peak density: ${max} notes/bar`)
  return parts.join(', ')
}

// ── Compact string serializer for one bar ─────────────────────────────────────
function barToCompactLine(bar) {
  const bn    = bar.bar_number ?? bar.bn
  const notes = bar.notes || []
  if (notes.length === 0) return `B${bn}:`
  const noteStrs = notes.map(n => {
    let s = `${getPitch(n)},${getStart(n)},${getDur(n)}`
    const o = getOffset(n)
    const c = getCutoff(n)
    if (o && o !== 0) s += `,o${o}`
    if (c != null)    s += `,c${c}`
  ...
  return `B${bn}: ${noteStrs.join('; ')}`
}

// ── Build note pattern sample in COMPACT STRING format ────────────────────────
// This is the most token-dense chunk — compact string saves ~80% vs JSON.
function buildNotePatternSample(bars, tempo, time_signature, spb) {
  if (!bars.length) return null
  ...
  // Include header so AI can parse the block as valid compact string
  lines.push(`H: ${tempo}|${time_signature}|?|${spb}`)
  lines.push(`OPENING (bars ${firstBn}–${lastBn}):`)
  opening.forEach(bar => lines.push(barToCompactLine(bar)))
  ...
  if (bars.length > 16) {
    ...
    if (mid.length > 0) {
      ...
      const midLastBn  = mid[mid.length - 1]?.bar_number ?? mid[mid.length - 1]?.bn ?? midStart + 4
      lines.push(`\nMID-SECTION (bars ${midFirstBn}–${midLastBn}):`)
      mid.forEach(bar => lines.push(barToCompactLine(bar)))
    }
  }
  ...
  if (bars.length > 20) {
    ...
    if (ending.length > 0) {
      ...
      const endLastBn  = ending[ending.length - 1]?.bar_number ?? ending[ending.length - 1]?.bn ?? bars.length
      lines.push(`\nENDING (bars ${endFirstBn}–${endLastBn}):`)
      ending.forEach(bar => lines.push(barToCompactLine(bar)))
    }
  }
...
function chunkMidi(json, sourceName) {
  ...
  chunks.push({
    ...
    text: [
      `METADATA [${handType.toUpperCase()}] "${sourceName}" piece:"${piece}".`,
      `Key:${json.key}  Tempo:${json.tempo}BPM  Time:${ts}  Bars:${bars.length}  spb:${spb}.`,
      stats ? `Notes:${stats.count}  Register:${stats.low}–${stats.high}  AvgDuration:${stats.avgDur}subdivisions.` : '',
      stats ? `Feel: ${stats.articulation}.` : '',
      stats?.hasSwing    ? 'Has swing/groove offset.' : '',
      stats?.hasStaccato ? 'Has staccato articulation.' : '',
      `Hand: ${handType === 'rh' ? 'RIGHT HAND — melody/upper voice.' : handType === 'lh' ? 'LEFT HAND — bass/accompaniment.' : 'FULL TRACK — both hands combined.'}`,
    ].filter(Boolean).join(' '),
    metadata: { source: sourceName, type: 'metadata', key: json.key, tempo: json.tempo },
  })

  // ── CHUNK 2: NOTE PATTERN (most valuable — compact string format) ──────────
  const patternSample = buildNotePatternSample(bars, json.tempo, ts, spb)
  if (patternSample) {
    chunks.push({
      id:   mkId(sourceName),
      text: [
        `NOTE_PATTERN [${handType.toUpperCase()}] "${sourceName}" piece:"${piece}":`,
        `Key:${json.key}  Tempo:${json.tempo}BPM  spb:${spb}`,
        `IMPORTANT: Study this pattern carefully. Mirror structure, rhythm density,`,
        `note spacing, and duration patterns. Do NOT copy notes verbatim.`,
        `Use as structural/rhythmic blueprint only.`,
        ``,
        patternSample,
  ...
  if (stats) {
    ...
    chunks.push({
      ...
      text: [
        ...
        `Register: ${stats.low} to ${stats.high}.`,
        `Articulation: ${stats.articulation}.`,
        stats.hasSwing    ? 'Timing: has swing/groove (use oNN on some notes).' : 'Timing: straight grid.',
        stats.hasStaccato ? 'Some notes are staccato (use cNN:50–75 on relevant notes).' : 'Mostly legato (omit c field).',
        `Density (first 16 bars): ${densitySnippet}.`,
        ``,
        handType === 'rh' ? [
          `RIGHT HAND COMPOSITION GUIDE for "${piece}":`,
