...
'use strict'
// backend/services/claudeService.js  v3.0
//
// ── WHAT CHANGED ──────────────────────────────────────────────────────────────
...
//  Drop-in replacement for geminiService — same exported API.
// ─────────────────────────────────────────────────────────────────────────────
...
async function compose(userPrompt, ragChunks, options = {}) {
  ...
  try {
    ...
    console.log(`${tag} ${ok} Composed — ${c.bold}${doc.bars?.length} bars${c.reset}  key:${c.cyan}${doc.key}${c.reset}  tempo:${c.cyan}${doc.tempo}${c.reset}  notes:${c.bold}${noteCount}${c.reset}`)
    return doc
  } catch (e) {
    console.error(`${tag} ${fail} Compact string parse failed\n${text.slice(0, 400)}`)
    throw new Error(`Claude returned unparseable compact string: ${e.message}`)
  }
}
...
async function retry(userPrompt, ragChunks, badDoc, errors) {
  ...
  const text = await callClaude(buildRetryPrompt(userPrompt, ragChunks, badStr, errors), 'retry')
  try {
    const doc = parseCompactStringResponse(text)
    console.log(`${tag} ${ok} Retry succeeded — ${doc.bars?.length} bars`)
    return doc
  } catch (e) {
    throw new Error(`Claude retry returned unparseable compact string: ${e.message}`)
  }
}
...
async function alterCompose(fullPromptText) {
  ...
  try {
    const doc       = parseCompactStringResponse(text)
    const noteCount = doc.bars?.reduce((s, b) => s + (b.notes?.length || 0), 0) || 0
    console.log(`${tag} ${ok} Alter parsed — ${c.bold}${doc.bars?.length} bars${c.reset}  added:${c.cyan}${noteCount}${c.reset}`)
    return doc
  } catch (e) {
