...
// 'use strict'
// // backend/services/geminiService.js  v2.0
// //
// // ── WHAT CHANGED ──────────────────────────────────────────────────────────────
// //  - REMOVED enhancePrompt() entirely.
// //
// //    Why: enhancePrompt made an extra API call that converted the user's prompt
// //    into a text "MUSICAL BLUEPRINT" (e.g. "Key: Am, melody in E4-E6 range...").
// //    This blueprint TEXT then replaced the actual user prompt and got fed into
// //    the compose prompt ALONGSIDE the reference note patterns.
// //
// //    The problem: the LLM prioritised the abstract descriptive blueprint over
// //    the concrete note_pattern JSON. Result → generic up-down scale motion
// //    that matched the blueprint description but ignored the real reference patterns.
// //
// //  - compose() now passes the user prompt DIRECTLY to buildComposePrompt().
// //    The note_pattern chunks in ragChunks do the job enhancePrompt was trying to do,
// //    but with actual musical data instead of descriptive text.
// //
// //  - Everything else (model fallback chain, JSON parsing, retry) unchanged.
// // ─────────────────────────────────────────────────────────────────────────────
...
// // NOTE: enhancePrompt() has been REMOVED.
// // The user prompt is passed directly — note_pattern chunks in ragChunks
// // provide concrete structural guidance without the blueprint text noise.
// function buildComposePrompt(userPrompt, ragChunks, options = {}) {
//   const hasRag = ragChunks && ragChunks.length > 0
...
//     console.log(`${tag} ${info} Mode: ${c.cyan}multi-section${c.reset} — section ${sectionIndex}/${totalSections}${hasRag ? ` + ${ragChunks.length} RAG chunks` : ''}`)
//   } else {
//     console.log(`${tag} ${info} Mode: ${c.cyan}single composition${c.reset}${hasRag ? ` + ${ragChunks.length} focused RAG chunks` : ' (no RAG context)'}`)
//   }
...
// async function compose(userPrompt, ragChunks, options = {}) {
//   // Direct compose — no enhancePrompt step.
//   // The note_pattern chunks in ragChunks provide concrete structural reference.
//   const promptText      = buildComposePrompt(userPrompt, ragChunks, options)
//   const { text, modelUsed } = await callWithFallback(promptText, 'compose')
...
'use strict'
// backend/services/geminiService.js  v3.0
//
// ── WHAT CHANGED ──────────────────────────────────────────────────────────────
//  AI now generates COMPACT STRING format instead of JSON.
//  parseCompactStringResponse() replaces parseJsonResponse().
//  compose/retry/alterCompose all return doc objects (parsed from compact string).
//  The doc objects are structurally compatible with jsonToMidi.convert().
//
//  Token savings on AI OUTPUT: ~80% reduction (C4,0,4 vs {"p":"C4","s":0,"d":4})
//  All musical guidance and RAG context logic unchanged.
// ─────────────────────────────────────────────────────────────────────────────

const fs   = require('fs')
const path = require('path')
const { GoogleGenerativeAI } = require('@google/generative-ai')
const { parseCompactString }  = require('./converters/stringToMidi')

// ── Terminal colours ───────────────────────────────────────────────────────────
const c = {
  reset: '\x1b[0m', dim: '\x1b[2m', green: '\x1b[32m', yellow: '\x1b[33m',
  cyan: '\x1b[36m', red: '\x1b[31m', bold: '\x1b[1m', gray: '\x1b[90m', magenta: '\x1b[35m',
}
const tag  = `${c.magenta}[GEMINI]${c.reset}`
const ok   = `${c.green}✔${c.reset}`
const warn = `${c.yellow}⚠${c.reset}`
...
function isRetryableError(err) {
  const msg = err?.message || ''
  return msg.includes('404') || msg.includes('429') || msg.includes('503')
...
async function callWithFallback(promptText, label = 'compose') {
  ...
  const key    = process.env.GEMINI_API_KEY
  const tokens = Math.round(promptText.length / 4)
  console.log(`${tag} ${info} ${label} — prompt ~${c.bold}${tokens} tokens${c.reset}`)
  ...
  for (const modelName of MODEL_CHAIN) {
    ...
    try {
      ...
      if (attempts < MODEL_CHAIN.length) {
        process.stdout.write(`${tag} ${c.dim}  waiting 1s…${c.reset}\n`)
        await new Promise(r => setTimeout(r, 1000))
      }
    }
  }
...
function _classifyError(err) {
  ...
  if (msg.includes('429'))                                return `${c.yellow}quota exceeded${c.reset}`
  if (msg.includes('404'))                                return `${c.yellow}model not available${c.reset}`
  if (msg.includes('503') || msg.includes('high demand')) return `${c.yellow}service overloaded${c.reset}`
  if (msg.includes('fetch failed'))                       return `${c.yellow}network error${c.reset}`
  if (msg.includes('401'))                                return `${c.red}invalid API key${c.reset}`
  return `${c.red}${msg.slice(0, 60)}${c.reset}`
}

// ── Extract compact string from AI response ────────────────────────────────────
// The AI outputs compact string format starting with "H: ..."
// Strip any markdown fences, then find and parse the H: block.
function parseCompactStringResponse(text) {
  // Strip markdown fences
  text = text.replace(/^```[a-z]*\s*/im, '').replace(/\s*```\s*$/, '').trim()

  // Find compact string block: starts with H: followed by header fields
  const match = text.match(/H:\s*[\d.]+\|[^\n|]+\|[^\n|]+\|\d+[\s\S]+/)
  if (match) {
    try {
      return parseCompactString(match[0].trim())
    } catch (e) {
      throw new Error(`Compact string parse error: ${e.message}\n---\n${match[0].slice(0, 300)}`)
    }
  }
  throw new Error(`No compact string (H: ...) found in AI response.\nFirst 400 chars:\n${text.slice(0, 400)}`)
}
...
function buildComposePrompt(userPrompt, ragChunks, options = {}) {
  ...
  if (userPrompt.includes('Output ONLY compact string') || userPrompt.includes('GENERATION COMPACT RULES')) {
    const ragContext = formatRagContext(ragChunks)
    return ragContext !== 'No reference material available.'
      ? `${userPrompt}\n\nADDITIONAL REFERENCE MATERIAL:\n${ragContext}`
      : userPrompt
  ...
  if (isMultiSection) {
    ...
    console.log(`${tag} ${info} Mode: ${c.cyan}single composition${c.reset}${hasRag ? ` + ${ragChunks.length} focused RAG chunks` : ' (no RAG)'}`)
  }
  ...
  const sectionNote = isMultiSection
    ...
    : ''
  const prevNote = previousSection
    ? `\n\nPREVIOUS SECTION HEADER:\nH: ${previousSection.tempo}|${previousSection.time_signature}|${previousSection.key}|${previousSection.subdivisions_per_bar}\nLast bar: ${previousSection.bars?.length || 0}\nContinue naturally from this.`
    : ''
...
function buildRetryPrompt(userPrompt, ragChunks, badStr, errors) {
  console.log(`${tag} ${warn} Retrying with ${errors.length} validation error(s) fed back`)
  return getContinuePrompt()
    .replace('{{USER_PROMPT}}', userPrompt.trim())
    .replace('{{RAG_CONTEXT}}', formatRagContext(ragChunks))
    .replace('{{BAD_OUTPUT}}',  badStr)
    .replace('{{ERRORS}}',      errors.join('\n'))
}
...
async function compose(userPrompt, ragChunks, options = {}) {
  ...
  try {
    const doc       = parseCompactStringResponse(text)
    const noteCount = doc.bars?.reduce((s, b) => s + (b.notes?.length || 0), 0) || 0
    console.log(`${tag} ${ok} Composed — ${c.bold}${doc.bars?.length} bars${c.reset}  key:${c.cyan}${doc.key}${c.reset}  tempo:${c.cyan}${doc.tempo}${c.reset}  notes:${c.bold}${noteCount}${c.reset}  model:${c.dim}${modelUsed}${c.reset}`)
    return doc
  } catch (e) {
    console.error(`${tag} ${fail} Compact string parse failed\n${text.slice(0, 400)}`)
    throw new Error(`Gemini returned unparseable compact string: ${e.message}`)
  }
}

async function retry(userPrompt, ragChunks, badDoc, errors) {
  // Reconstruct a compact string snippet from the bad doc for context
  let badStr = '[unparseable output]'
  try {
    const { serializeToCompactString } = require('./converters/midiToString')
    badStr = serializeToCompactString(badDoc).slice(0, 800)
  } catch (_) { badStr = JSON.stringify(badDoc).slice(0, 800) }

  const promptText = buildRetryPrompt(userPrompt, ragChunks, badStr, errors)
  const { text }   = await callWithFallback(promptText, 'retry')

  try {
    const doc = parseCompactStringResponse(text)
    console.log(`${tag} ${ok} Retry succeeded — ${doc.bars?.length} bars`)
    return doc
  } catch (e) {
    throw new Error(`Gemini retry returned unparseable compact string: ${e.message}`)
  }
}
...
async function alterCompose(fullPromptText) {
  ...
  try {
    const doc       = parseCompactStringResponse(text)
    const noteCount = doc.bars?.reduce((s, b) => s + (b.notes?.length || 0), 0) || 0
    console.log(`${tag} ${ok} Alter parsed — ${c.bold}${doc.bars?.length} bars${c.reset}  added:${c.cyan}${noteCount}${c.reset}  model:${c.dim}${modelUsed}${c.reset}`)
    return doc
  } catch (e) {
    console.error(`${tag} ${fail} Alter compact string parse failed\n${text.slice(0, 400)}`)
    throw new Error(`Gemini returned unparseable compact string: ${e.message}`)
  }
}
