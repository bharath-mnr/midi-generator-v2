// //E:\pro\midigenerator_v2\backend\converters\jsonToMidi.js
...
// ── Normalise a bar from compact (bn) or full (bar_number) format ─────────────
...
function jsonToMidiEvents(json) {
  ...
  for (const rawBar of bars) {
    ...
    for (const note of bar.notes) {
      const mp  = pitchToMidi(note.pitch)
      const vel = 100  // always fixed — never read from input
