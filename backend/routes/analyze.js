...
// backend/routes/analyze.js
//
// Updated for v3.0 engine — universal, data-driven analysis
// No hardcoded music-type assumptions.
//
// Endpoints (all under /api/analyze):
//   POST /api/analyze          — full analysis
//   POST /api/analyze/patterns — families only (lightweight)
//   POST /api/analyze/yaml     — YAML blueprint only
//
// Body for all three: { json: <MIDI_JSON_object_or_string>, splitMidi?: <number> }
...
// ─── FAMILIES ONLY (lightweight) ──────────────────────────────────────
// Returns just the pattern families + window sizes, no full labeled bars
router.post('/patterns', (req, res, next) => {
  try {
    const { json: inputJson, splitMidi } = req.body
    if (!inputJson) return res.status(400).json({ error: 'Missing json field' })
    ...
    res.json({
      success: true,
      families: sanitize(result.rightHand.families || []),
      windowSizes: sanitize(result.rightHand.windowSizes || []),
      boundaries: sanitize(result.rightHand.boundaries || []),
    })
  } catch (err) {
    next(err)
  }
