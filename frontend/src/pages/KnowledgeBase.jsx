...
// Multi-file upload support — select or drop many files at once
// Files are uploaded sequentially with a progress counter
...
// ── DropZone — multi-file aware ───────────────────────────────────────────────
// onFiles(fileArray) receives ALL selected/dropped files at once
function DropZone({ accept, label, hint, color, glow, onFiles, uploading, progress }) {
  const [over, setOver] = useState(false)
  const ref = useRef(null)
  ...
  const handleDrop = e => {
    e.preventDefault()
    setOver(false)
    // Get ALL dropped files (not just [0])
    const files = Array.from(e.dataTransfer.files)
    if (files.length > 0) onFiles(files)
  }

  const handleChange = e => {
    // Get ALL selected files (not just [0])
    const files = Array.from(e.target.files)
    if (files.length > 0) onFiles(files)
    e.target.value = ''   // reset so same files can be re-selected
  }

  const isActive = uploading || over
  ...
  return (
    <div
      ...
      style={{
        padding: '20px 16px',
        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10,
        border: `1px dashed ${isActive ? color : 'var(--line-md)'}`,
        borderRadius: 12,
        background: isActive ? glow : 'var(--surface)',
        cursor: uploading ? 'wait' : 'pointer',
        transition: 'all 0.15s', userSelect: 'none',
      }}
      onMouseEnter={e => { if (!uploading) { e.currentTarget.style.borderColor = color; e.currentTarget.style.background = glow } }}
      onMouseLeave={e => { if (!uploading && !over) { e.currentTarget.style.borderColor = 'var(--line-md)'; e.currentTarget.style.background = 'var(--surface)' } }}
    >
      {/* multiple attribute — the key fix */}
      <input
        ref={ref}
        type="file"
        accept={accept}
      ...
      <div style={{ textAlign: 'center' }}>
        <div style={{
          ...
          color: uploading ? color : 'var(--tx-1)',
          marginBottom: 3,
        }}>
          {uploading && progress
      ...
      {/* Progress bar — only visible when uploading multiple */}
    ...
    >
      ...
      <button
        ...
        onMouseEnter={e => { if (!deleting) {
          e.currentTarget.style.borderColor = 'rgba(245,85,74,0.45)'
...
export default function KnowledgeBase() {
  ...
  // Separate progress state for MIDI and Doc zones
  const [midiProgress, setMidiProgress] = useState(null)   // null | { current, total, name }
  const [docProgress,  setDocProgress]  = useState(null)

  const [deletingId, setDeletingId] = useState(null)
  const [uploadErr,  setUploadErr]  = useState(null)

  // ── Upload a batch of files sequentially, one at a time ──
  // This is sequential (not parallel) because the backend processes one at a time
  // and parallel uploads would overload the server on free tier
  const uploadBatch = async (files, ingestFn, setProgress) => {
    setUploadErr(null)
    const errors = []
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        // Collect errors but keep going through remaining files
  ...
  const handleMidiFiles = (files) => uploadBatch(files, ingestMidi, setMidiProgress)
  const handleDocFiles  = (files) => uploadBatch(files, ingestDoc,  setDocProgress)
    ...
    <div className="page">
      ...
      <div className="grid-2" style={{ marginBottom: 20 }}>
        <DropZone
          ...
          hint=".mid · .midi · select multiple"
          color="var(--lime)"
          glow="rgba(184,245,74,0.06)"
          onFiles={handleMidiFiles}
          uploading={!!midiProgress}
          progress={midiProgress}
        />
        <DropZone
          accept=".pdf,.txt,.md"
          label="Upload Documents"
          hint=".pdf · .txt · .md · select multiple"
          color="var(--sky)"
          glow="rgba(74,184,245,0.06)"
      ...
      {uploadErr && (
        <div style={{
          ...
          background: 'var(--rose-glow)', border: '1px solid rgba(245,85,74,0.25)',
          borderRadius: 8, padding: '9px 13px', marginBottom: 16,
          fontFamily: 'var(--mono)', whiteSpace: 'pre-line',
          display: 'flex', gap: 8, alignItems: 'flex-start',
        }}>
          <span>⚠</span>
          <span style={{ flex: 1 }}>{uploadErr}</span>
          <button
            onClick={() => setUploadErr(null)}
            style={{ background: 'none', border: 'none', color: 'var(--rose)', cursor: 'pointer', fontSize: 14, lineHeight: 1 }}
          >✕</button>
        </div>
      )}
      ...
      {/* ── Error from hook ──────────────────────────────────────────────────── */}
      {error && (
        <div style={{ fontSize: 12, color: 'var(--rose)', fontFamily: 'var(--mono)', marginBottom: 10 }}>
          ⚠ {error}
        </div>
      )}
      ...
      {/* ── List ────────────────────────────────────────────────────────────── */}
      {loading ? (
        <div style={{ fontSize: 11, color: 'var(--tx-3)', fontFamily: 'var(--mono)', padding: '20px 0' }}>
          Loading…
        </div>
      ) : items.length === 0 ? (
        <div style={{ padding: '48px 0', textAlign: 'center', color: 'var(--tx-3)', fontSize: 13 }}>
          No files indexed yet. Upload MIDI or documents above.
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {items.map(item => (
