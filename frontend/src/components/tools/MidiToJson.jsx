...
// frontend/src/components/tools/MidiToJson.jsx
...
class Engine {
  ...
  static convertToJson(midiData) {
    ...
    // Fill gaps so empty bars are represented
    const filledBars = []
    if (bars.length > 0) {
  ...
  static convert(buf) {
    const m = this.parseMidiFile(buf)
    return this.convertToJson(m)
  }
}
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setStats({
        ...
        notes: json.bars.reduce((a, b) => a + b.notes.length, 0),
        tempo: json.tempo,
      })
    } catch (e) {
      setErrors([e.message])
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        >
          <input
            ref={ref} type="file" accept=".mid,.midi"
            style={{ display: 'none' }}
            onChange={e => { setFile(e.target.files[0]); setOutput(''); setErrors([]); setStats(null) }}
          />
          <Upload size={24} stroke={file ? 'var(--accent)' : 'var(--text3)'} />
          <div style={{ marginTop: 10, fontSize: 13, color: file ? 'var(--accent)' : 'var(--text2)' }}>
            {file ? file.name : 'Click to upload .mid file'}
          </div>
          <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 4 }}>.mid · .midi</div>
        </div>
        ...
        >
          <Activity size={14} />
          {processing ? 'Converting...' : 'Convert to Compact JSON'}
        </button>

        {/* Compact format reminder */}
        <div style={{
          background: 'var(--surface2)', border: '1px solid var(--border)',
          borderRadius: 'var(--radius-sm)', padding: '10px 14px',
          fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text3)', lineHeight: 1.8,
        }}>
          <span style={{ color: 'var(--text2)', fontWeight: 600 }}>Output compact fields: </span>
          ...
          <span style={{ color: 'var(--text3)' }}>o</span>? (omitted if 0)&nbsp;&nbsp;
          <span style={{ color: 'var(--text3)' }}>c</span>? (omitted if null)&nbsp;&nbsp;
          <span style={{ color: 'rgba(245,85,74,0.8)' }}>velocity stripped</span> (always 100)
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <textarea
          ...
          placeholder="Upload a MIDI file and convert — compact JSON (p/s/d, no velocity) appears here..."
          style={{
            flex: 1, minHeight: 280, width: '100%',
            background: 'var(--surface2)', border: '1px solid var(--border)',
        ...
        {stats && (
          <StatusBar
            status="ok"
            message={`${stats.bars} bars · ${stats.notes} notes · ${stats.tempo} BPM · velocity stripped`}
          />
        )}
      </div>
    </div>
