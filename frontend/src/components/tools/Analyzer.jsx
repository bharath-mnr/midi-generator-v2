...
// // frontend/src/components/tools/Analyzer.jsx
// //
// // v3.0 — updated adaptResponse() for new engine shape.
// // All UI components unchanged (TimelineRow, PatternCard, GraphView, etc.)
...
// // ═══════════════════════════════════════════════════════════════
// // RESPONSE ADAPTER  — maps v3 engine output → UI component props
// // ═══════════════════════════════════════════════════════════════

// function adaptResponse(data) {
//   const meta = data.metadata

//   // v3 uses "families" where v2 used "patterns" — both shapes exposed
...
//   // Normalize labeled bars — add .color + ensure .alternating shape
//   const normLabeled = (labeled) => (labeled || []).map(lb => ({
//     ...lb,
//     color: lb.isBoundaryEnd ? BND_CLR : (patColorMap.get(lb.patternId) || SUR_CLR),
//     // v3 already puts alternating as { pedal, notesPerBar } or null
...
//       ...s,
//       pid:   s.patternId,
//       color: colorFrom(fam) || (s.patternId === 'MIXED' ? '#6b7280' : SUR_CLR),
//     }
//   })

//   // Normalize patterns for PatternCard — flatten color object → hex string
//   const normPatterns = (families) => (families || []).map(f => ({
//     ...f,
//     color:         colorFrom(f),
//     // v2-compat fields PatternCard expects:
...
//   const rhLabeled  = normLabeled(data.rightHand.labeled)
//   const lhLabeled  = normLabeled(data.leftHand.labeled)
//   const rhSections = normSections(data.rightHand.sections, rhFamilies)
//   const lhSections = normSections(data.leftHand.sections, lhFamilies)
...
//                 )}
//                 {lb.isBoundaryEnd && (
//                   <div style={{ position:'absolute', right:0, top:0, bottom:0, width:2, background:'rgba(255,255,255,0.8)', borderRadius:1 }} />
//                 )}
//               </div>
//             ))}
//           </div>
...
//           <span style={{ color:'var(--tx-1)', fontWeight:600 }}>Bar {hov.barNumber}</span>
//           {' · '}
//           <span style={{ color: hov.isBoundaryEnd ? BND_CLR : hov.isSurprise ? SUR_CLR : hov.isEmpty ? 'var(--tx-3)' : hov.color }}>
//             {hov.patternLabel}
//           </span>
//           {' · '}{hov.noteCount} notes
...
//             const stroke = e.weight >= maxW * 0.6 ? clr : '#374151'
//             return (
//               <g key={i}>
//                 <path d={`M${f.x},${f.y} Q${mx},${my} ${t.x},${t.y}`} fill="none" stroke={stroke} strokeWidth={sw} strokeOpacity={0.55} markerEnd="url(#ar)" />
//                 <text x={mx} y={my - 3} textAnchor="middle" fill="#6b7280" fontSize="8">{e.weight}</text>
...
//                 {aln.rh?.alternating  && ' ⟳'}
//                 {aln.rh?.isSustain    && ' ▬'}
//                 {aln.rh?.variationType ? ` (${aln.rh.variationType})` : ''}
//               </div>
//               <div style={{ padding:'3px 8px', borderRadius:4, fontSize:10, fontFamily:'var(--mono)', background:lhC+'22', color:lhC, border:`1px solid ${lhC}33`, overflow:'hidden', whiteSpace:'nowrap', textOverflow:'ellipsis' }}>
//                 {aln.lh?.patternLabel || '—'}
//                 {aln.lh?.alternating  && ' ⟳'}
...
//   return (
//     <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
//       <div style={{ fontSize:10, fontFamily:'var(--mono)', color:'var(--tx-3)', marginBottom:4 }}>
//         Natural window sizes: [{windowSizes.join(', ')}]
//       </div>
...
//             <div style={{ color:BND_CLR, fontWeight:600, marginBottom:4 }}>
//               After bar {b.afterBarNumber} — {b.type}
//             </div>
//             <div style={{ color:'var(--tx-3)', fontSize:10 }}>
//               novelty: {b.noveltyScore?.toFixed(3)}
//             </div>
//             {b.dims && (
//               <div style={{ marginTop:4, display:'flex', gap:4, flexWrap:'wrap' }}>
//                 {Object.entries(b.dims).map(([dim, val]) => (
//                   <span key={dim} style={{ fontSize:9, padding:'1px 5px', borderRadius:3, background:'var(--surface3)', color:'var(--tx-2)' }}>
//                     {dim}:{val.toFixed(2)}
//                   </span>
//                 ))}
//               </div>
...
//             </span>
//             <input ref={fileRef} type="file" accept=".json,.txt" style={{ display:'none' }} onChange={handleFile} />
//             <button onClick={() => fileRef.current?.click()} style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 10px', borderRadius:'var(--radius-sm)', border:'1px solid var(--border)', background:'none', color:'var(--tx-2)', cursor:'pointer', fontSize:11, fontWeight:600 }}>
//               <Upload size={11} /> Load file
//             </button>
//           </div>
//         </div>
...
//           onClick={runAnalyze}
//           disabled={running || !input.trim() || !jsonOk}
//           style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6, padding:'9px 18px', borderRadius:'var(--radius-sm)', background: running||!input.trim()||!jsonOk ? 'var(--surface3)' : 'var(--accent)', color: running||!input.trim()||!jsonOk ? 'var(--tx-3)' : '#000', fontSize:12, fontWeight:700, cursor: running||!input.trim()||!jsonOk ? 'not-allowed' : 'pointer', border:'none', transition:'all 0.15s ease', alignSelf:'flex-start' }}
//         >
//           <Activity size={13} />
//           {running ? 'Analyzing…' : 'Analyze'}
//         </button>
...
//                       Right hand — {res.rhPatterns.length} pattern families
//                     </div>
//                     <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:10 }}>
//                       {res.rhPatterns.map(p => <PatternCard key={p.id} pat={p} meta={meta} spb={meta.subdivisions_per_bar} />)}
//                     </div>
...
//                       Left hand — {res.lhPatterns.length} pattern families
//                     </div>
//                     <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:10 }}>
//                       {res.lhPatterns.map(p => <PatternCard key={p.id} pat={p} meta={meta} spb={meta.subdivisions_per_bar} />)}
//                     </div>
